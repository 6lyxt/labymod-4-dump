...
public class VersionedChatModifier implements InternalChatModifier
{
    ...
    public void deleteMessage(final ChatMessage message) {
        final VersionedGuiNewChat accessor = (VersionedGuiNewChat)ave.A().q.d();
        final Predicate<ava> messageFilter = this.messageFilter(message);
    ...
    public void editMessage(final ChatMessage message, final Component component) {
        ...
        final ava prevMessage = accessor.getMessages().get(index);
        final eu mapped = (eu)new VersionedChatMessageComponent(message, (eu)this.componentMapper.toMinecraftComponent(component));
        accessor.getMessages().set(index, new ava(prevMessage.b(), mapped, prevMessage.c()));
        int first = -1;
    ...
    private Predicate<ava> messageFilter(final ChatMessage message) {
