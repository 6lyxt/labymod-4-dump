...
public class MixinNetHandlerPlayServer
{
    ...
    private void labyMod$fireDisconnectEventPre(final eu reason, final InsertInfo ci) {
        final GameProfile profile = (GameProfile)this.b.cd();
        Laby.fireEvent(new IntegratedServerPlayerDisconnectEvent(Phase.PRE, profile));
    }
    
    @Insert(method = { "onDisconnect" }, at = @At("TAIL"))
    private void labyMod$fireDisconnectEventPost(final eu reason, final InsertInfo ci) {
        final GameProfile profile = (GameProfile)this.b.cd();
