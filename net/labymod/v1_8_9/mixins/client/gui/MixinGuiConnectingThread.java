...
public class MixinGuiConnectingThread
{
    ...
    public eu labyMod$fireServerKickEvent(final eu reason) {
        final ServerData serverData = Laby.labyAPI().serverController().createServerData(ave.A().D());
        if (serverData == null) {
            return reason;
        }
        return (eu)Laby.fireEvent(new ServerKickEvent(ConnectableServerData.from(serverData), (Component)reason, ServerKickEvent.Context.PRE_LOGIN)).reason();
    }
}
