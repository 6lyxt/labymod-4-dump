...
public abstract class MixinScreen implements FileDropHandler, ScreenAccessor
{
    ...
    public void fireScreenInitEvent(final enn mc, final int width, final int height, final InsertInfo ci) {
        Laby.fireEvent(new VersionedScreenInitEvent(this));
    }
    
    @Insert(method = { "renderBackground" }, at = @At("HEAD"), cancellable = true)
    public void renderThemeBackground(final eox graphics, final InsertInfo ci) {
    ...
    public void renderThemeDirtBackground(final eox graphics, final InsertInfo ci) {
        this.renderThemeBackground(((VanillaStackAccessor)graphics.c()).stack(), ci);
    }
    ...
    private void renderThemeBackground(Stack stack, final InsertInfo ci) {
        ...
        stack = ((stack != null) ? stack : ((VanillaStackAccessor)new eij()).stack());
    ...
    private void labyMod$disableMenuBackground(final eox graphics, final InsertInfo ci) {
    ...
    public void renderComponentHoverEffect(final Stack stack, final Style style, final int x, final int y) {
        if (this.labyMod$graphics == null) {
            this.labyMod$graphics = new eox(this.f, this.f.aN().b());
        }
        ((GuiGraphicsAccessor)this.labyMod$graphics).setPose((eij)stack.getProvider().getPoseStack());
        this.labyMod$graphics.a(this.i, (ts)style, x, y);
    }
    
    @Override
    public void setMinecraft(final enn minecraft) {
        this.f = minecraft;
    }
    ...
    public List<Object> getWrappedWidgets() {
        ...
        for (final eqt child : this.k) {
            wrappedWidgets.add(this.asVersionedWidget(child));
        }
        return wrappedWidgets;
    }
    
    private VersionedWidget asVersionedWidget(final eqt child) {
        ...
        if (child instanceof final eqs container) {
            for (final eqt entry : container.i()) {
