...
import net.labymod.v1_20_1.client.renderer.OverlayRenderType;
...
public class MixinLevelRenderer implements WorldRenderer
{
    ...
    public void labyMod$renderCustomOutline(final fjv instance, final eij poseStack, final ein vertexConsumer, final bfj entity, final double x, final double y, final double z, final gu blockPos, final dcb blockState) {
        ...
        final efb voxelShape = blockState.a((cls)this.x, blockPos, een.a(entity));
        ...
        if (event.getOverlayColor() != null) {
            final ein boxVertex = this.w.b().getBuffer((fkf)OverlayRenderType.INSTANCE);
    ...
    private eey.a drawLines(final ein vertexConsumer, final eij.a pose, final double x, final double y, final double z, final Color color) {
        ...
        return (minX, minY, minZ, maxX, maxY, maxZ) -> {
            ...
            final float offset = apa.c(blockX * blockX + blockY * blockY + blockZ * blockZ);
            blockX /= offset;
            blockY /= offset;
            blockZ /= offset;
    ...
    private void labyMod$beforeEntityRenderPass(final eij param0, final float param1, final long param2, final boolean param3, final emz param4, final fjq param5, final fjw param6, final Matrix4f param7, final CallbackInfo ci) {
    ...
    private void labyMod$afterEntityRenderPass(final eij param0, final float param1, final long param2, final boolean param3, final emz param4, final fjq param5, final fjw param6, final Matrix4f param7, final CallbackInfo ci) {
        Laby.fireEvent(new EntityRenderPassEvent(EntityRenderPassEvent.Phase.AFTER));
    }
    
    private eey.a drawBoxes(final ein vertexConsumer, final eij.a pose, final double x, final double y, final double z, final Color color) {
        final float red = color.getRed() / 255.0f;
        final float green = color.getGreen() / 255.0f;
        final float blue = color.getBlue() / 255.0f;
        final float alpha = color.getAlpha() / 255.0f;
    ...
    private ein vertex(final ein vertexConsumer, final Matrix4f matrix, final double x, final double y, final double z) {
