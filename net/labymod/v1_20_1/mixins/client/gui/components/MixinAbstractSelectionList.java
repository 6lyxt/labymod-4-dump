...
public abstract class MixinAbstractSelectionList
{
    ...
    public void labyMod$renderBackground(final eox graphics, final int param1, final int param2, final float param3, final InsertInfo ci) {
    ...
    public void labyMod$renderTopAndBottom(final eox graphics, final int param1, final int param2, final float param3, final InsertInfo ci) {
        final Theme theme = Laby.labyAPI().themeService().currentTheme();
        if (!theme.metadata().getBoolean("hide_default_background")) {
    ...
    public void labyMod$renderList(final epc instance, final eox graphics, final int mouseX, final int mouseY, final float partialTicks, final Operation<Void> original) {
        final Scissor scissor = Laby.labyAPI().gfxRenderPipeline().scissor();
        try {
            scissor.push(((VanillaStackAccessor)graphics.c()).stack(), (float)this.f, (float)(this.g - this.f));
