...
public abstract class MixinMouseHandler implements MouseHandlerAccessor
{
    ...
    private void redirectPlayerRotation(final fiy player, final double x, final double y) {
    ...
    public void grabMouseNative() {
        ehe.a(this.a.aM().i(), 212995, this.a.aM().m() / 2.0, this.a.aM().n() / 2.0);
    }
    
    @Override
    public void ungrabMouseNative() {
        ehe.a(this.a.aM().i(), 212993, this.a.aM().m() / 2.0, this.a.aM().n() / 2.0);
    }
    
    @Redirect(method = { "onScroll" }, at = @At(value = "FIELD", ordinal = 5, target = "Lnet/minecraft/client/MouseHandler;accumulatedScroll:D"))
    private double labyMod$fireHotbarSlotChangeEvent(final eno instance) {
        final double delta = this.p;
        if (delta == 0.0) {
            return 0.0;
        }
        final fiy player = this.a.t;
        final int slotDelta = MathHelper.clamp((int)delta, -1, 1);
        final ClientHotbarSlotChangeEvent event = new ClientHotbarSlotChangeEvent(player.fN().l, slotDelta);
