...
public class VersionedChatModifier implements InternalChatModifier
{
    ...
    public void fireChatClear(final boolean clearHistory) {
        enn.N().l.d().a(clearHistory);
    }
    
    @Override
    public void deleteMessage(final ChatMessage message) {
        final VersionedChatComponent accessor = (VersionedChatComponent)enn.N().l.d();
    ...
    public void editMessage(final ChatMessage message, final Component component) {
        ...
        final enh prevMessage = accessor.getMessages().get(index);
        final sw mapped = (sw)new VersionedChatMessageComponent(message, (sw)this.componentMapper.toMinecraftComponent(component));
        accessor.getMessages().set(index, new enh(prevMessage.a(), mapped, prevMessage.c(), prevMessage.d()));
        final Predicate<enh.a> predicate = this.formattedFilter(message);
        int first = -1;
    ...
    private Predicate<enh> messageFilter(final ChatMessage message) {
    ...
    private Predicate<enh.a> formattedFilter(final ChatMessage message) {
