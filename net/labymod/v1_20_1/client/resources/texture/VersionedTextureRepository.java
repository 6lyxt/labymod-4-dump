...
public class VersionedTextureRepository extends DefaultAbstractTextureRepository implements TextureRepository
{
    ...
    public void onTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof final PathResourceLocation path) {
                final fuw textureManager = this.getTextureManager();
                final acq minecraftLocation = resourceLocation.getMinecraftLocation();
    ...
    public void register(final ResourceLocation location, final Texture texture, @Nullable final Runnable finishHandler) {
        final fug minecraftTexture = this.convertToVersionedTexture(texture);
    ...
    public void registerTexture(final ResourceLocation location, final Object texture, final Runnable finishHandler) {
        if (texture instanceof final fug minecraftTexture) {
        ...
        throw new IllegalStateException(texture.getClass().getName() + " is not a subtype of " + fug.class.getName());
    }
    
    @Override
    public void preloadTexture(final ResourceLocation location) {
        this.registerTexture(location, (fug)new fuo((acq)location.getMinecraftLocation()));
    }
    ...
    public Texture getTexture(final ResourceLocation location) {
        ...
        final fug texture = this.getTextureManager().b((acq)location.getMinecraftLocation());
    ...
    private void release(final ResourceLocation location, final Runnable releaseHandler) {
        this.getTextureManager().c((acq)location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull final ResourceLocation location, final fug texture) {
        this.getTextureManager().a((acq)location.getMinecraftLocation(), texture);
    }
    
    private void registerTexture(@NotNull final ResourceLocation location, final fug texture, final Runnable finishHandler) {
    ...
    private fug convertToVersionedTexture(final Texture texture) {
        if (texture instanceof final LabyTexture labyTexture) {
            return (fug)new VersionedLabyTexture(labyTexture);
        }
        if (texture instanceof final fug abstractTexture) {
            return abstractTexture;
        }
        throw this.createSubtypeException(texture, fug.class);
    }
}
