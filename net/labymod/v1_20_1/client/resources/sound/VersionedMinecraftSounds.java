...
public class VersionedMinecraftSounds implements MinecraftSounds
{
    ...
    public void playChatFilterSound() {
        enn.N().ah().a((fxy)fxt.a((he)amh.qp, 1.0f));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch) {
        this.playSound(fxt.a(new amg((acq)location.getMinecraftLocation(), 16.0f, false), pitch, volume));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch, final FloatVector3 position) {
        this.playSound(new fxt((acq)location.getMinecraftLocation(), ami.a, volume, pitch, fxy.t(), false, 0, fxy.a.a, (double)position.getX(), (double)position.getY(), (double)position.getZ(), true));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch, final DoubleVector3 position) {
        this.playSound(new fxt((acq)location.getMinecraftLocation(), ami.a, volume, pitch, fxy.t(), false, 0, fxy.a.a, position.getX(), position.getY(), position.getZ(), true));
    }
    
    private void playSound(final fxt sound) {
        sound.a(enn.N().ah());
        enn.N().ah().a((fxy)sound);
    }
}
