...
public enum LegacyKeyIdConverter
{
    ...
    private final int legacyKeyId;
    
    private LegacyKeyIdConverter(final String name, final int key) {
        this(name, key, -1);
    }
    
    private LegacyKeyIdConverter(final String name, final int keyId, final int legacyKeyId) {
        this.name = name;
        this.legacyKeyId = legacyKeyId;
    }
    
    public static String getKeyName(final int legacyKeyId) {
        return LegacyKeyIdConverter.ID_TO_NAME.getOrDefault(legacyKeyId, LegacyKeyIdConverter.UNKNOWN.name);
    }
    
    public static int getKeyId(final String name) {
        return (LegacyKeyIdConverter.NAME_TO_ID.get(name) == null) ? -1 : LegacyKeyIdConverter.NAME_TO_ID.get(name);
    }
    
    static {
        ID_TO_NAME = new HashMap<Integer, String>();
        NAME_TO_ID = new HashMap<String, Integer>();
        for (final LegacyKeyIdConverter value : values()) {
            LegacyKeyIdConverter.NAME_TO_ID.put(value.name, value.legacyKeyId);
            LegacyKeyIdConverter.ID_TO_NAME.put(value.legacyKeyId, value.name);
        }
    }
}
