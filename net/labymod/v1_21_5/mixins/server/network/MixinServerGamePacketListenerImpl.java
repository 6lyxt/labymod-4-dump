...
public class MixinServerGamePacketListenerImpl
{
    ...
    private void labyMod$fireDisconnectEventPre(final xg reason, final InsertInfo ci) {
        final GameProfile profile = (GameProfile)this.f.gi();
        Laby.fireEvent(new IntegratedServerPlayerDisconnectEvent(Phase.PRE, profile));
    }
    
    @Insert(method = { "onDisconnect" }, at = @At("TAIL"))
    private void labyMod$fireDisconnectEventPost(final xg reason, final InsertInfo ci) {
        final GameProfile profile = (GameProfile)this.f.gi();
