...
public abstract class MixinChunkAccess implements Chunk
{
    ...
    @Shadow
    public abstract ehp a(final ehp.a p0);
    
    @Shadow
    public abstract edy b(final int p0);
    ...
    public Heightmap heightmap(final HeightmapType type) {
        ...
        return (Heightmap)this.a(ehp.a.b);
    }
    ...
    public BlockState getBlockState(final int x, final int y, final int z) {
        final int index = ((edn)this).f(y);
        if (index < 0 || index >= this.m.length) {
            return (BlockState)dnq.a.m();
        }
        final edy section = this.m[index];
        if (section == null) {
            return (BlockState)dnq.a.m();
        }
    ...
    public BlockEntity[] getBlockEntities() {
        ...
        int index = 0;
        for (final Map.Entry<iw, dyo> entry : this.k.entrySet()) {
    ...
    public int getHeightBasedOnSection(final int startY) {
        final edy[] sections = this.d();
        if (sections.length == 0) {
            return 0;
        }
        final int bottomY = this.K_();
        ...
        for (int index = startSectionIndex; index < sections.length; ++index) {
            final edy section = sections[index];
            if (section != null) {
                if (!section.c()) {
