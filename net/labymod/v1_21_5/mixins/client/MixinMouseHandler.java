...
public abstract class MixinMouseHandler implements MouseHandlerAccessor
{
    ...
    private void redirectPlayerRotation(final gqm player, final double x, final double y) {
    ...
    public void grabMouseNative() {
        fka.a(this.b.aO().h(), 212995, this.b.aO().m() / 2.0, this.b.aO().n() / 2.0);
    }
    
    @Override
    public void ungrabMouseNative() {
        fka.a(this.b.aO().h(), 212993, this.b.aO().m() / 2.0, this.b.aO().n() / 2.0);
    }
    ...
    private Vector2i labyMod$fireHotbarSlotChangeEvent(final fqz instance, final double $$0, final double $$1, final Operation<Vector2i> original) {
        final Vector2i mouseScroll = (Vector2i)original.call(new Object[] { instance, $$0, $$1 });
        if (mouseScroll.y == 0) {
            return mouseScroll;
        }
        final gqm player = this.b.t;
        final int slotDelta = MathHelper.clamp(mouseScroll.y, -1, 1);
        final ClientHotbarSlotChangeEvent event = new ClientHotbarSlotChangeEvent(player.gj().f(), slotDelta);
