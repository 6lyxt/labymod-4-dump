...
public class VersionedMinecraftSounds implements MinecraftSounds
{
    ...
    public void playChatFilterSound() {
        fqq.Q().ak().a((hon)hoi.a((jg)awy.sK, 1.0f));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch) {
        this.playSound(hoi.a(awx.a((alr)location.getMinecraftLocation(), 16.0f), pitch, volume));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch, final FloatVector3 position) {
        this.playSound(new hoi((alr)location.getMinecraftLocation(), awz.a, volume, pitch, hon.t(), false, 0, hon.a.a, (double)position.getX(), (double)position.getY(), (double)position.getZ(), true));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch, final DoubleVector3 position) {
        this.playSound(new hoi((alr)location.getMinecraftLocation(), awz.a, volume, pitch, hon.t(), false, 0, hon.a.a, position.getX(), position.getY(), position.getZ(), true));
    }
    
    private void playSound(final hoi sound) {
        sound.a(fqq.Q().ak());
        fqq.Q().ak().a((hon)sound);
    }
}
