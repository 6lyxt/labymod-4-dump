...
public class ButtonConverter extends AbstractMinecraftWidgetConverter<fto, ButtonWidget>
{
    ...
    @Override
    public ButtonWidget createDefault(final fto source) {
        final ButtonWidget destination = ButtonWidget.component((source instanceof fvd.b) ? null : this.mapComponent(source.B()), null, Pressable.NOOP);
    ...
    @Override
    public void update(final fto source, final ButtonWidget destination) {
    ...
    @Override
    public String getWidgetId(final fto source) {
        return this.componentMapper.getTranslationKeyOfComponent(source.B());
    }
    
    @Override
    public List<String> getWidgetIds(final fto source) {
    ...
    {
        @Override
        public ButtonWidget map(final fto source) {
            if (source instanceof final fuk imageButton) {
                ...
                final Icon icon = Icon.sprite(location, accessor.getXTexStart(), accessor.getYTexStart(), source.A(), source.y(), accessor.getTextureWidth(), accessor.getTextureHeight());
    ...
    class LockIconButtonMapper implements Mapper<fto, ButtonWidget>
    {
        ...
        @Override
        public ButtonWidget map(final fto source) {
            if (source instanceof final fuo lockIconButton) {
                final int size = source.A();
