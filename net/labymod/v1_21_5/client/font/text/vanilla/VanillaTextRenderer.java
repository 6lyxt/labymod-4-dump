...
package net.labymod.v1_21_5.client.font.text.vanilla;
...
import net.labymod.v1_21_5.client.font.DynamicStringRenderOutput;
...
public class VanillaTextRenderer extends AbstractTextRenderer
{
    ...
    public float width(final int codepoint, final Style style) {
        final fra.f provider = ((StringSplitterAccessor)this.font().b()).getWidthProvider();
        if (style == null || style.isEmpty()) {
            return provider.getWidth(codepoint, yd.a);
        }
        return provider.getWidth(codepoint, (yd)style);
    }
    ...
    public float width(final VisualCharSequence text) {
        return (float)this.font().a((azk)text.castTo(azk.class));
    }
    ...
    public TextBuffer render(final Stack stack) {
        ...
        if (this.isAlphaAboveThreshold(color)) {
            ...
            if (!hasMultiBufferSource) {
                multiBufferSource = fqq.Q().aR().c();
            }
            stringWidth = this.renderComponent(stack, (grn)multiBufferSource, component, x, y, color);
            if (!hasMultiBufferSource) {
                ((grn.a)multiBufferSource).b();
            }
        }
        else {
    ...
    public void beginBatch(final Stack stack) {
        if (stack.getMultiBufferSource() == null) {
            stack.multiBufferSource(fqq.Q().aR().c());
    ...
    private float renderComponent(final Stack stack, final grn bufferSource, final RenderableComponent component, final float x, final float y, final int color) {
    ...
    public float getWidth(final RenderableComponent component) {
        ...
        float finalWidth = 0.0f;
        for (final FormattedRenderableComponent formattedRenderableComponent : formattedRenderableComponents) {
            final azk formattedText = this.composite(formattedRenderableComponent);
    ...
    private void renderFormattedComponent(final Matrix4f modelViewMatrix, final DynamicStringRenderOutput output, final FormattedRenderableComponent component, final float x, final float y, final int color) {
        for (final RenderableComponent child : component.components()) {
            final azk sequence = MinecraftUtil.fromText(child.getText(), child.style());
    ...
    private void renderText(final DynamicStringRenderOutput output, final azk sequence, final float x, final float y, final int color, final boolean dropShadow, final Matrix4f modelViewMatrix) {
