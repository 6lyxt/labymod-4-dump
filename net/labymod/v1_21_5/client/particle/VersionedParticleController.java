...
public class VersionedParticleController implements ParticleController
{
    ...
    public void crackBlock(final FloatVector3 blockPosition, final Direction direction) {
        final iw position = new iw(MathHelper.ceil(blockPosition.getX()), MathHelper.ceil(blockPosition.getY()), MathHelper.ceil(blockPosition.getZ()));
        fqq.Q().g.a(position, this.getMinecraftDirection(direction));
    }
    
    public jc getMinecraftDirection(final Direction direction) {
        switch (direction) {
            case DOWN: {
                return jc.a;
            }
