...
public class DynamicFont extends ekm implements FontHeightAccessor
{
    ...
    private final ekm delegate;
    private final ejs customSplitter;
    
    public DynamicFont(final ekm delegate) {
        super(delegate.f, delegate.g);
        this.delegate = delegate;
        this.customSplitter = new ejs((codepoint, style) -> DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().width(codepoint, (Style)style));
    }
    
    @NotNull
    public emu a(@NotNull final acf location) {
    ...
    public int b(final String text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final Matrix4f modelViewMatrix, @NotNull final fdv bufferSource, final boolean seeThrough, final int backgroundColor, final int packedLightCoords, final boolean bidirectionalShaping) {
    ...
    public int b(@NotNull final ano text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final Matrix4f modelViewMatrix, @NotNull final fdv bufferSource, final boolean seeThrough, final int backgroundColor, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
    ...
    public void a(@NotNull final ano text, final float x, final float y, final int color, final int outlineColor, @NotNull final Matrix4f modelViewMatrix, @NotNull final fdv bufferSource, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
        ...
        builder.pos(x, y).text(labyComponent).drawMode(MinecraftUtil.fromMinecraft(ekm.a.c)).color(color).shadow(false).render(stack);
        stack.pop();
    }
    ...
    public int a(@NotNull final sw text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
    ...
    public int a(@NotNull final ano text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
        final StringOutput stringOutput = new StringOutput();
        text.accept((anp)stringOutput);
        return this.a((sw)DynamicFont.COMPONENT_MAPPER.toMinecraftComponent(stringOutput.getLabyComponent()));
    }
    ...
    public String a(@NotNull final String text, final int maxWidth, final boolean tail) {
        ...
        if (tail) {
            return this.customSplitter.c(text, maxWidth, to.a);
        }
        return this.customSplitter.b(text, maxWidth, to.a);
    }
    ...
    @NotNull
    public sw a(@NotNull final sw text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text, maxWidth);
        }
        return this.customSplitter.a(text, maxWidth, to.a);
    }
    ...
    public int b(@NotNull final String text, final int maxWidth) {
        ...
        final int lines = this.customSplitter.g(text, maxWidth, to.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    public int b(@NotNull final sw text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.b(text, maxWidth);
        }
        final int lines = this.customSplitter.b(text, maxWidth, to.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    @NotNull
    public List<ano> c(@NotNull final sw text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.c(text, maxWidth);
        }
        return qk.a().a(this.customSplitter.b(text, maxWidth, to.a));
    }
    ...
    static class StringOutput implements anp
    {
        ...
        public boolean accept(final int position, @NotNull final to style, final int codepoint) {
