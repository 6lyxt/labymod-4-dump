...
public class VersionedChatModifier implements InternalChatModifier
{
    ...
    public void fireChatClear(final boolean clearHistory) {
        ejf.N().l.d().a(clearHistory);
    }
    
    @Override
    public void deleteMessage(final ChatMessage message) {
        final VersionedChatComponent accessor = (VersionedChatComponent)ejf.N().l.d();
    ...
    public void editMessage(final ChatMessage message, final Component component) {
        ...
        final eja prevMessage = accessor.getMessages().get(index);
        final ss mapped = (ss)new VersionedChatMessageComponent(message, (ss)this.componentMapper.toMinecraftComponent(component));
        accessor.getMessages().set(index, new eja(prevMessage.a(), mapped, prevMessage.c(), prevMessage.d()));
        final Predicate<eja.a> predicate = this.formattedFilter(message);
        int first = -1;
    ...
    private Predicate<eja> messageFilter(final ChatMessage message) {
    ...
    private Predicate<eja.a> formattedFilter(final ChatMessage message) {
