...
package net.labymod.v1_19_3.mixins.server.network;
...
public class MixinServerGamePacketListenerImpl
{
    ...
    private void labyMod$fireDisconnectEventPre(final ss reason, final InsertInfo ci) {
        final GameProfile profile = (GameProfile)this.c.fD();
        Laby.fireEvent(new IntegratedServerPlayerDisconnectEvent(Phase.PRE, profile));
    }
    
    @Insert(method = { "onDisconnect" }, at = @At("TAIL"))
    private void labyMod$fireDisconnectEventPost(final ss reason, final InsertInfo ci) {
        final GameProfile profile = (GameProfile)this.c.fD();
