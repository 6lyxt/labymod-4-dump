...
import net.labymod.v1_19_3.client.util.WatchablePlayerInfoSet;
...
public abstract class MixinClientPacketListener implements ClientPacketListener
{
    ...
    @Shadow
    public abstract void a(final ss p0);
    ...
    private void labyMod$fireDimensionChangeEvent(final wu packet, final InsertInfo ci) {
        final acf from = this.m.ac().a();
        final acf to = packet.c().a();
        Laby.fireEvent(new DimensionChangeEvent(ResourceLocation.create(from.b(), from.a()), ResourceLocation.create(to.b(), to.a())));
    }
    
    @Inject(method = { "<init>" }, at = { @At("TAIL") })
    private void labyMod$watchableListedPlayers(final ejf $$0, final epb $$1, final sb $$2, final ezg $$3, final GameProfile $$4, final fte $$5, final CallbackInfo ci) {
        this.p = new WatchableHashSet<eze>(new WatchablePlayerInfoSet(this.labyMod$shownPlayers));
    }
    ...
    private void labyMod$disableWarn(final wn $$0, final CallbackInfo ci) {
        ci.cancel();
    }
    
    @Inject(method = { "handlePlayerInfoUpdate" }, at = { @At("TAIL") })
    private void labyMod$handleAddPlayer(final wn packet, final CallbackInfo ci) {
        for (final wn.b entry : packet.d()) {
            final UUID profileId = entry.a();
            if (this.labyMod$playerInfos.containsKey(profileId)) {
                continue;
            }
            final NetworkPlayerInfo playerInfo = new VersionedNetworkPlayerInfo(this.o.get(profileId));
            this.labyMod$playerInfos.put(profileId, playerInfo);
            Laby.fireEvent(new PlayerInfoAddEvent(playerInfo));
        }
        for (final wn.b entry : packet.c()) {
            final NetworkPlayerInfo playerInfo2 = this.labyMod$playerInfos.get(entry.a());
            if (playerInfo2 == null) {
                continue;
            }
            for (final wn.a action : packet.b()) {
    ...
    private void labyMod$handleClassicPvPBlocking(final xl packet, final InsertInfo insertInfo) {
        final int entityId = packet.b();
        final bdr entity = this.m.a(entityId);
        if (!(entity instanceof bwp)) {
            return;
        }
        final bwp player = (bwp)entity;
        final List<abq.b<?>> unpackedData = packet.c();
        ...
        unpackedData.stream().filter(entry -> entry.a() == 8).forEach(entry -> {
            ...
            if (!(!(value instanceof Byte))) {
                final bcl activeHand = player.fa();
                final int state = (int)value;
                final SwordOldAnimation animation = LabyMod.getInstance().getOldAnimationRegistry().get("sword");
                if (animation != null) {
                    try {
                        final cdt heldItem = player.b(activeHand);
                        if (heldItem.c() == cdw.tU || heldItem.c() instanceof cfc) {
    ...
    public ss labyMod$modifyReason(final ss component) {
        ...
        return (ss)componentMapper.toMinecraftComponent(Laby.fireEvent(new ServerKickEvent(serverData, componentMapper.fromMinecraftComponent(component), ServerKickEvent.Context.PLAY)).reason());
    }
    
    @Inject(method = { "send(Lnet/minecraft/network/protocol/game/ServerboundResourcePackPacket$Action;)V" }, at = { @At("HEAD") })
    private void labyMod$disableCustomFont(final zz.a action, final CallbackInfo ci) {
        if (action == zz.a.a) {
    ...
    private void labyMod$fireAbilityUpdateEvent(final wh packet, final CallbackInfo ci) {
        Laby.fireEvent(new ClientPlayerAbilitiesUpdateEvent((ClientPlayer)ejf.N().t));
    }
    ...
    public void simulateKick(final Component reason) {
        ...
        this.a((ss)Laby.labyAPI().minecraft().componentMapper().toMinecraftComponent(reason));
    }
}
