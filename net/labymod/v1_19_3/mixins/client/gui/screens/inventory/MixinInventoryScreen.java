...
@Mixin({ eqx.class })
public abstract class MixinInventoryScreen extends eqp<caf>
{
    private InventorySlotRegistry labyMod$inventorySlotRegistry;
    private boolean labyMod$oldSurvivalLayoutTexture;
    
    public MixinInventoryScreen(final caf param0, final bwo param1, final ss param2) {
        super((bzg)param0, param1, param2);
    }
    ...
    private void labyMod$init(final InsertInfo info) {
        ...
        for (final cax slot : ((caf)this.x()).i) {
    ...
    private void labyMod$setOldSurvivalLayoutTexture(final int slot, final acf location) {
        if (this.labyMod$oldSurvivalLayoutTexture) {
            RenderSystem.setShaderTexture(slot, (acf)Textures.SURVIVAL_INVENTORY_BACKGROUND.getMinecraftLocation());
            return;
        }
    ...
    @NotNull
    private cax getSlotByIndex(@NotNull final InventoryLayoutOldAnimation inventoryLayout, @NotNull final cax slot) {
