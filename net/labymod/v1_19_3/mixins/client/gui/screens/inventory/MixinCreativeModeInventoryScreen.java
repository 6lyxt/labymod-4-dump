...
package net.labymod.v1_19_3.mixins.client.gui.screens.inventory;
...
@Mixin({ eqn.class })
public abstract class MixinCreativeModeInventoryScreen extends eqp<eqn.b>
{
    ...
    public MixinCreativeModeInventoryScreen(final eqn.b pickerMenu, final bwo inventory, final ss component) {
        super((bzg)pickerMenu, inventory, component);
    }
    ...
    private void labyMod$setEntityToOldPosition(final int x, final int y, final int scale, final float mouseX, final float mouseY, final beg entity) {
        if (this.labyMod$oldCreativeLayoutTexture) {
            eqx.a(x - this.labyMod$entityXShift, y - this.labyMod$entityYShift, scale, mouseX - this.labyMod$entityXShift, mouseY - this.labyMod$entityYShift, entity);
            return;
        }
        eqx.a(x, y, scale, mouseX, mouseY, entity);
    }
    ...
    private void labyMod$setOldCreativeTexture(final int slot, final acf location) {
        if (!this.labyMod$oldCreativeLayoutTexture) {
        ...
        if (resourceLocation != null) {
            RenderSystem.setShaderTexture(slot, (acf)resourceLocation.getMinecraftLocation());
        }
    }
    ...
    private void labyMod$onSelectTab(final ccd tab, final CallbackInfo ci) {
        this.labyMod$modifySlots();
    }
    
    private void labyMod$modifySlots() {
        if (this.labyMod$oldCreativeLayoutTexture) {
            for (final cax slot : ((eqn.b)this.x()).i) {
    ...
    @NotNull
    private cax getSlotByIndex(@NotNull final InventoryLayoutOldAnimation inventoryLayout, @NotNull final InventorySlotData legacySlot, @NotNull final cax slot) {
