...
public class MixinConnectScreenThread
{
    ...
    private Optional<fab> labyMod$resolveAddress(final fae resolver, final fac address) {
        final InetSocketAddress resolved = ConnectAddressResolver.resolve(address.a(), address.b(), (AddressResolveCallback)this.d);
        return (resolved == null) ? Optional.empty() : Optional.of(fab.a(resolved));
    }
    ...
    public ss labyMod$fireServerKickEvent(final ss reason) {
        ...
        return (ss)componentMapper.toMinecraftComponent(Laby.fireEvent(new ServerKickEvent(ConnectableServerData.from(data), componentMapper.fromMinecraftComponent(reason), ServerKickEvent.Context.PRE_LOGIN)).reason());
    }
}
