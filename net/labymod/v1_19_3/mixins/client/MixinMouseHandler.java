...
public abstract class MixinMouseHandler implements MouseHandlerAccessor
{
    ...
    private void labyMod$handleMouseScroll(final long windowHandle, final double xOffset, final double yOffset, final InsertInfo callback) {
        final ejj options = ejf.N().m;
        final double scrollOffset = options.L().c() ? Math.signum(yOffset) : yOffset;
        final double delta = scrollOffset * (double)options.z().c();
    ...
    private void redirectPlayerRotation(final fcz player, final double x, final double y) {
    ...
    public void grabMouseNative() {
        ecy.a(this.a.aN().i(), 212995, this.a.aN().m() / 2.0, this.a.aN().n() / 2.0);
    }
    
    @Override
    public void ungrabMouseNative() {
        ecy.a(this.a.aN().i(), 212993, this.a.aN().m() / 2.0, this.a.aN().n() / 2.0);
    }
    
    @Redirect(method = { "onScroll" }, at = @At(value = "FIELD", ordinal = 5, target = "Lnet/minecraft/client/MouseHandler;accumulatedScroll:D"))
    private double labyMod$fireHotbarSlotChangeEvent(final ejg instance) {
        final double delta = this.p;
        if (delta == 0.0) {
            return 0.0;
        }
        final fcz player = this.a.t;
        final int slotDelta = MathHelper.clamp((int)delta, -1, 1);
        final ClientHotbarSlotChangeEvent event = new ClientHotbarSlotChangeEvent(player.fE().k, slotDelta);
        ...
        if (event.toSlot() != originalToSlot && !player.B_()) {
            player.fE().k = MathHelper.clamp(event.toSlot(), 0, 8);
            return 0.0;
        }
        return delta;
    }
