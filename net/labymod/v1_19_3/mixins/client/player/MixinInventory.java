...
import net.labymod.v1_19_3.client.player.inventory.InventoryTracker;
...
public class MixinInventory implements Inventory
{
    ...
    public ItemStack itemStackAt(final int index) {
        try {
            final cdt itemStack = (cdt)this.h.get(index);
            return MinecraftUtil.fromMinecraft(itemStack);
        }
        catch (final IndexOutOfBoundsException exception) {
            return MinecraftUtil.fromMinecraft(cdw.a.T_());
        }
    }
    
    @Override
    public void setCreativeModeItemStack(final int index, final ItemStack itemStack) {
        final cdt stack = MinecraftUtil.toMinecraft(itemStack);
        final ezd gameMode = ejf.N().r;
        final fcz player = ejf.N().t;
        if (gameMode == null || player == null || !player.fF().d || stack.b()) {
            return;
        }
    ...
    public int countAllArrows() {
        int found = 0;
        for (final hi<cdt> list : this.n) {
            for (final cdt stack : list) {
                if (stack.a(amk.aq)) {
                    found += stack.K();
                }
            }
    ...
    public cdt removeItem(final List<cdt> list, final int index, final int amount) {
        final cdt itemStack = bcg.a((List)list, index, amount);
