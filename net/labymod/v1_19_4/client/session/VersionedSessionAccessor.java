...
public class VersionedSessionAccessor extends DefaultAbstractSessionAccessor<emw>
{
    ...
    @Override
    public emw toGameSession(final Session session) {
        return (emw)this.userFactory.createUser(session);
    }
    
    @Override
    public Session fromGameSession(final emw gameSession) {
        return (gameSession == null) ? null : this.createSession(gameSession.c(), gameSession.h().getId(), gameSession.d());
    }
    
    @Override
    public emw getGameSession() {
        return emh.N().U();
    }
}
