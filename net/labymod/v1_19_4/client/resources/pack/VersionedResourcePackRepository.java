...
public class VersionedResourcePackRepository extends AbstractResourcePackRepository<akq>
{
    @Inject
    ...
    public void registerSilentPack(final ResourcePack pack) {
        ...
        final alh resourceManager = emh.N().Y();
        if (resourceManager instanceof final ale reloadableResourceManager) {
    ...
    @Override
    public void onRebuildSelected(final List<akq> selected) {
        this.selectedPacks.clear();
        this.setHasServerPackSelected(false);
        for (final akq pack : selected) {
            this.selectedPacks.add(pack.f());
            this.checkServerPack(pack);
        }
    }
    
    private void checkServerPack(final akq pack) {
        try (final ajv resources = pack.e()) {
            final ajv unwrappedPackResources = PackUtil.unwrap(resources);
            if (!(unwrappedPackResources instanceof aju)) {
                if (resources != null) {
                    resources.close();
                }
                return;
    ...
    protected void onReloadPackDetails() {
        ...
        final Collection<akq> packs = emh.N().Z().c();
        for (final akq pack : packs) {
            final ResourcePackDetail detail = ResourcePackDetail.create(pack.f(), pack.a(), pack.b());
            try (final ajv resources = pack.e()) {
                final akz<InputStream> resource = (akz<InputStream>)resources.a(new String[] { "pack.png" });
                if (resource != null) {
                ...
                final ajv unrwappedPackResources = PackUtil.unwrap(resources);
