...
public class VersionedTextureRepository extends DefaultAbstractTextureRepository implements TextureRepository
{
    ...
    public void onTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof final PathResourceLocation path) {
                final ftd textureManager = this.getTextureManager();
                final add minecraftLocation = resourceLocation.getMinecraftLocation();
    ...
    public void register(final ResourceLocation location, final Texture texture, @Nullable final Runnable finishHandler) {
        final fsn minecraftTexture = this.convertToVersionedTexture(texture);
    ...
    public void registerTexture(final ResourceLocation location, final Object texture, final Runnable finishHandler) {
        if (texture instanceof final fsn minecraftTexture) {
        ...
        throw new IllegalStateException(texture.getClass().getName() + " is not a subtype of " + fsn.class.getName());
    }
    
    @Override
    public void preloadTexture(final ResourceLocation location) {
        this.registerTexture(location, (fsn)new fsv((add)location.getMinecraftLocation()));
    }
    ...
    public Texture getTexture(final ResourceLocation location) {
        ...
        final fsn texture = this.getTextureManager().b((add)location.getMinecraftLocation());
    ...
    private void release(final ResourceLocation location, final Runnable releaseHandler) {
        this.getTextureManager().c((add)location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull final ResourceLocation location, final fsn texture) {
        this.getTextureManager().a((add)location.getMinecraftLocation(), texture);
    }
    
    private void registerTexture(@NotNull final ResourceLocation location, final fsn texture, final Runnable finishHandler) {
    ...
    private fsn convertToVersionedTexture(final Texture texture) {
        if (texture instanceof final LabyTexture labyTexture) {
            return (fsn)new VersionedLabyTexture(labyTexture);
        }
        if (texture instanceof final fsn abstractTexture) {
            return abstractTexture;
        }
        throw this.createSubtypeException(texture, fsn.class);
    }
}
