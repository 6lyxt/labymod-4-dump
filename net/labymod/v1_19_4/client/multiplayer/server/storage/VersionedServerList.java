...
public class VersionedServerList extends DefaultServerList
{
    ...
    public void load() {
        try {
            ...
            final re root = ro.b(new File(this.gameDirectory, "servers.dat"));
            if (root == null) {
                return;
            }
            final rk list = root.c("servers", 10);
    ...
    private re serverDataToTag(final StorageServerData data) {
        final re tag = new re();
        ...
        if (data.hasMetadata()) {
            ...
            tag.a("labyModMetadata", (rx)metaTag);
        }
        return tag;
    }
    
    private StorageServerData tagToServerData(final re tag) {
        ...
        if (tag.b("labyModMetadata", 10)) {
            final re metaTag = tag.p("labyModMetadata");
            for (final String key : metaTag.e()) {
                final rx metaValue = metaTag.c(key);
                String value;
                if (metaValue instanceof rv) {
                    value = metaValue.f_();
                }
                else {
                    if (!(metaValue instanceof rb)) {
                        continue;
                    }
                    final rb byteArrayTag = (rb)metaValue;
