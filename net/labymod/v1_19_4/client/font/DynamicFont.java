...
public class DynamicFont extends enp implements FontHeightAccessor
{
    ...
    private final enp delegate;
    private final emt customSplitter;
    
    public DynamicFont(final enp delegate) {
        super(delegate.f, delegate.g);
        this.delegate = delegate;
        this.customSplitter = new emt((codepoint, style) -> DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().width(codepoint, (Style)style));
    }
    
    @NotNull
    public eqe a(@NotNull final add location) {
    ...
    public int b(final String text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final Matrix4f modelViewMatrix, @NotNull final fig bufferSource, @NotNull final enp.a displayMode, final int backgroundColor, final int packedLightCoords, final boolean bidirectionalShaping) {
    ...
    public int b(@NotNull final aov text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final Matrix4f modelViewMatrix, @NotNull final fig bufferSource, @NotNull final enp.a displayMode, final int backgroundColor, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
    ...
    public void a(@NotNull final aov text, final float x, final float y, final int color, final int outlineColor, @NotNull final Matrix4f modelViewMatrix, @NotNull final fig bufferSource, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
        ...
        builder.pos(x, y).text(labyComponent).drawMode(MinecraftUtil.fromMinecraft(enp.a.c)).color(color).shadow(false).render(stack);
        stack.pop();
    }
    ...
    public int a(@NotNull final tn text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
    ...
    public int a(@NotNull final aov text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
        final StringOutput stringOutput = new StringOutput();
        text.accept((aow)stringOutput);
        return this.a((tn)DynamicFont.COMPONENT_MAPPER.toMinecraftComponent(stringOutput.getLabyComponent()));
    }
    ...
    public String a(@NotNull final String text, final int maxWidth, final boolean tail) {
        ...
        if (tail) {
            return this.customSplitter.c(text, maxWidth, uf.a);
        }
        return this.customSplitter.b(text, maxWidth, uf.a);
    }
    ...
    @NotNull
    public tn a(@NotNull final tn text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text, maxWidth);
        }
        return this.customSplitter.a(text, maxWidth, uf.a);
    }
    ...
    public int b(@NotNull final String text, final int maxWidth) {
        ...
        final int lines = this.customSplitter.g(text, maxWidth, uf.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    public int b(@NotNull final tn text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.b(text, maxWidth);
        }
        final int lines = this.customSplitter.b(text, maxWidth, uf.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    @NotNull
    public List<aov> c(@NotNull final tn text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.c(text, maxWidth);
        }
        return qz.a().a(this.customSplitter.b(text, maxWidth, uf.a));
    }
    ...
    static class StringOutput implements aow
    {
        ...
        public boolean accept(final int position, @NotNull final uf style, final int codepoint) {
