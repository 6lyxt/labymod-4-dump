...
package net.labymod.v1_19_4.client.font.text.vanilla;
...
import net.labymod.v1_19_4.client.font.DynamicStringRenderOutput;
...
public class VanillaTextRenderer extends AbstractTextRenderer
{
    ...
    public float width(final int codepoint, final Style style) {
        final emt.f provider = ((StringSplitterAccessor)this.font().b()).getWidthProvider();
        if (style == null || style.isEmpty()) {
            return provider.getWidth(codepoint, uf.a);
        }
        return provider.getWidth(codepoint, (uf)style);
    }
    ...
    public float width(final VisualCharSequence text) {
        return (float)this.font().a((aov)text.castTo(aov.class));
    }
    ...
    public TextBuffer render(final Stack stack) {
        ...
        if (this.isAlphaAboveThreshold(color)) {
            ...
            if (!hasMultiBufferSource) {
                multiBufferSource = emh.N().aN().b();
            }
            stringWidth = this.renderComponent(stack, (fig)multiBufferSource, component, x, y, color);
            if (!hasMultiBufferSource) {
                ((fig.a)multiBufferSource).b();
            }
        }
        else {
    ...
    public void beginBatch(final Stack stack) {
        if (stack.getMultiBufferSource() == null) {
            stack.multiBufferSource(emh.N().aN().b());
    ...
    public void endBatch(final Stack stack) {
        if (this.bufferSourceAdded) {
            ...
            if (multiBufferSource instanceof final fig.a bufferSource) {
                bufferSource.b();
            }
    ...
    private float renderComponent(final Stack stack, final fig bufferSource, final RenderableComponent component, final float x, final float y, final int color) {
    ...
    public float getWidth(final RenderableComponent component) {
        ...
        float finalWidth = 0.0f;
        for (final FormattedRenderableComponent formattedRenderableComponent : formattedRenderableComponents) {
            final aov formattedText = this.composite(formattedRenderableComponent);
    ...
    private void renderFormattedComponent(final Matrix4f modelViewMatrix, final DynamicStringRenderOutput output, final FormattedRenderableComponent component, final float x, final float y, final int color) {
        for (final RenderableComponent child : component.components()) {
            final aov sequence = MinecraftUtil.fromText(child.getText(), child.style());
    ...
    private void renderText(final DynamicStringRenderOutput output, final aov sequence, final float x, final float y, final int color, final boolean dropShadow, final Matrix4f modelViewMatrix) {
        ...
        if (dropShadow) {
            ...
            sequence.accept((aow)output);
            copiedModelViewMatrix.translate((Vector3fc)enp.e);
        }
