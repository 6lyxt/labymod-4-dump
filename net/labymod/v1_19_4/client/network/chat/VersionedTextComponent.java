...
public class VersionedTextComponent extends VersionedBaseComponent<TextComponent, tk> implements TextComponent
{
    ...
    public VersionedTextComponent(final tw holder, final boolean empty) {
        super(holder);
        this.empty = empty;
    }
    ...
    public String getText() {
        ...
        if (contents instanceof final uo literal) {
            return literal.a();
        }
    ...
    public TextComponent text(final String text) {
        if (this.empty) {
            ((MutableComponentAccessor)this.holder).setContents((tk)new uo(text));
            this.empty = false;
            return this;
        }
