...
public abstract class MixinMouseHandler implements MouseHandlerAccessor
{
    ...
    private void labyMod$handleMouseScroll(final long windowHandle, final double xOffset, final double yOffset, final InsertInfo callback) {
        final eml options = emh.N().m;
        final double scrollOffset = options.N().c() ? Math.signum(yOffset) : yOffset;
        final double delta = scrollOffset * (double)options.B().c();
    ...
    private void redirectPlayerRotation(final fhk player, final double x, final double y) {
    ...
    public void grabMouseNative() {
        efz.a(this.a.aM().i(), 212995, this.a.aM().m() / 2.0, this.a.aM().n() / 2.0);
    }
    
    @Override
    public void ungrabMouseNative() {
        efz.a(this.a.aM().i(), 212993, this.a.aM().m() / 2.0, this.a.aM().n() / 2.0);
    }
    
    @Redirect(method = { "onScroll" }, at = @At(value = "FIELD", ordinal = 5, target = "Lnet/minecraft/client/MouseHandler;accumulatedScroll:D"))
    private double labyMod$fireHotbarSlotChangeEvent(final emi instance) {
        final double delta = this.p;
        if (delta == 0.0) {
            return 0.0;
        }
        final fhk player = this.a.t;
        final int slotDelta = MathHelper.clamp((int)delta, -1, 1);
        final ClientHotbarSlotChangeEvent event = new ClientHotbarSlotChangeEvent(player.fJ().l, slotDelta);
