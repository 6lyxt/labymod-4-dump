...
public class MixinKeyBindsScreen
{
    ...
    private emf[] labyMod$checkKeys(final eml instance) {
        final List<emf> mappings = new ArrayList<emf>(Arrays.asList(instance.X));
        for (final Hotkey hotkey : HotkeyHolder.getInstance().getHotkeys()) {
            mappings.add(LabyModKeyMapping.create(hotkey));
        }
        return mappings.toArray(new emf[0]);
    }
    ...
    private emf[] labyMod$resetKeys(final eml instance) {
        final List<emf> mappings = new ArrayList<emf>(Arrays.asList(instance.X));
        for (final Hotkey hotkey : HotkeyHolder.getInstance().getHotkeys()) {
            mappings.add(LabyModKeyMapping.create(hotkey));
        }
