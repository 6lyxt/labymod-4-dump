...
import net.labymod.v1_19_4.client.gui.overlay.CustomLoadingOverlay;
...
public abstract class MixinMinecraft
{
    ...
    private String labyMod$censorSessionId(final emw instance) {
        return "********************************";
    }
    
    @Redirect(method = { "<init>" }, at = @At(value = "FIELD", target = "Lnet/minecraft/client/main/GameConfig$UserData;proxy:Ljava/net/Proxy;"))
    private Proxy labyMod$callGameInitializeResource(final eyl.d userData) {
    ...
    private File labyMod$onPreGameStarted(final eyl.a instance) {
    ...
    private egi labyMod$adjustWindowSize(final fiv instance, efw displayData, final String $$1, final String $$2) {
        ...
        if (sizePair != null) {
            displayData = new efw((int)sizePair.getFirst(), (int)sizePair.getSecond(), displayData.c, displayData.d, displayData.e);
        }
    ...
    private void labyMod$maximizeDisplay(final eyl $$0, final CallbackInfo ci) {
    ...
    private void labyMod$firePostGameStartedInitializeEvent(final eyl param0, final CallbackInfo ci) {
    ...
    private String labyMod$modifiedTitle(final StringBuilder stringBuilder) {
        ...
        if (listener != null && listener.g().h()) {
            ...
            if (this.aN != null && !this.aN.p()) {
                ...
                final fdq currentServer = this.P();
    ...
    private void labyMod$cancelRemovedScreen(final etd screen) {
    }
    ...
    private ScreenWrapper labyMod$createScreenWrapper(final etd screen) {
    ...
    private esq labyMod$createLoadingOverlay(final emh minecraft, final ald reloadInstance, final Consumer onFinish, final boolean fadeIn) {
        ...
        return (type == FadeOutAnimationType.FADING && !DynamicBackgroundController.isEnabled()) ? new esq(minecraft, reloadInstance, onFinish, fadeIn) : new CustomLoadingOverlay(minecraft, reloadInstance, onFinish, fadeIn);
    }
    
    @Insert(method = { "clearLevel(Lnet/minecraft/client/gui/screens/Screen;)V" }, at = @At("HEAD"))
    private void labyMod$firePreNetworkDisconnectEvent(final etd screen, final InsertInfo ci) {
    ...
    private void labyMod$firePostNetworkDisconnectEvent(final etd screen, final InsertInfo ci) {
    ...
    private void labyMod$firePreWorldLoadEvent(final fdj level, final InsertInfo ci) {
        if (level != null) {
    ...
    private void labyMod$firePostWorldLoadEvent(final fdj level, final InsertInfo ci) {
        if (level != null) {
    ...
    private void labyMod$fireGameRenderEvent(final fhz gameRenderer, final float partialTicks, final long nanoTime, final boolean running) {
        final ehe poseStack = new ehe();
    ...
    private void labyMod$fireGameRenderEvent(final Phase phase, final Stack stack, final float partialTicks) {
        final ehe modelViewStack = RenderSystem.getModelViewStack();
        modelViewStack.a();
    ...
    public static void c(final o report) {
        ...
        adf.a(report.e());
        if (report.f() != null) {
            adf.a("#@!@# Game crashed! Crash report saved to: #@!@# " + String.valueOf(report.f()));
            System.exit(-1);
        }
        else if (report.a(crashFile)) {
            adf.a("#@!@# Game crashed! Crash report saved to: #@!@# " + crashFile.getAbsolutePath());
            System.exit(-1);
        }
        else {
            adf.a("#@?@# Game crashed! Crash report could not be saved. #@?@#");
            System.exit(-2);
        }
    }
}
