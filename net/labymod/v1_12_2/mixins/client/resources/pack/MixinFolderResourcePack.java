...
public abstract class MixinFolderResourcePack extends ced implements ResourceLister
{
    ...
    public void listResources(final String namespace, final String baseDirectory, final Consumer<nf> locationConsumer) {
        ...
        try {
            ...
            try (final Stream<Path> files = Files.find(startPath, Integer.MAX_VALUE, (path, attributes) -> attributes.isRegularFile(), new FileVisitOption[0])) {
                files.forEach(resource -> {
                    ...
                    locationConsumer.accept(new nf(namespace, path));
                    return;
                });
            }
        }
