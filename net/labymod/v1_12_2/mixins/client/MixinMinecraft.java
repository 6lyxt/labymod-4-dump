...
public abstract class MixinMinecraft
{
    ...
    private String labyMod$censorSessionId(final bii instance) {
        return "********************************";
    }
    ...
    private char labyMod$fixScreenshotKeyBug() {
        return (char)(Keyboard.getEventCharacter() - '\u0100');
    }
    ...
    private void labyMod$refreshChatForAdvancedItemTooltips(final int lvt_1_1_, final CallbackInfoReturnable<Boolean> cir) {
    ...
    private void labyMod$firePostStarupInitializeEvent(final String s, final CallbackInfo ci) {
    ...
    private ScreenWrapper labyMod$createScreenWrapper(final blk screen) {
    ...
    public void labyMod$overrideInitScreen(final blk screen, final bib mc, final int width, final int height) {
    ...
    private void labyMod$disconnectFromServer(final String p_launchIntegratedServer_1_, final String p_launchIntegratedServer_2_, final amx p_launchIntegratedServer_3_, final CallbackInfo ci) {
        final bsb world = this.f;
        if (world != null) {
            world.O();
        }
    }
    ...
    private void labyMod$fireLoadWorldEvent(final bsb world, final String name, final CallbackInfo ci) {
    ...
    private void labyMod$firePostNetworkDisconnectEvent(final bsb world, final String name, final CallbackInfo ci) {
    ...
    public void c(final b report) {
        final File dir = new File(z().w, "crash-reports");
        ...
        ni.a(report.e());
        if (report.f() != null) {
            ni.a("#@!@# Game crashed! Crash report saved to: #@!@# " + String.valueOf(report.f()));
            System.exit(-1);
        }
        else if (report.a(crashFile)) {
            ni.a("#@!@# Game crashed! Crash report saved to: #@!@# " + crashFile.getAbsolutePath());
            System.exit(-1);
        }
        else {
            ni.a("#@?@# Game crashed! Crash report could not be saved. #@?@#");
            System.exit(-2);
        }
    }
    
    private void labyMod$fireLoadWorldEventPhase(final bsb worldClient, final Phase phase) {
        if (worldClient == null) {
    ...
    private boolean labyMod$handleSendClickBlockToController(final bsa instance, final et blockPos, final fa facing) {
        return !this.labyMod$fireClientPlayerInteractEvent(ClientPlayerInteractEvent.InteractionType.ATTACK) && instance.b(blockPos, facing);
    }
    ...
    private void labyMod$fireGameRenderEvent(final buq gameRenderer, final float partialTicks, final long nanoTime) {
    ...
    @Inject(method = { "toggleFullscreen" }, at = { @At(value = "INVOKE", target = "Lorg/lwjgl/opengl/Display;setFullscreen(Z)V", shift = At.Shift.AFTER, remap = false) })
    private void labyMod$enableResizeAfterFullscreen(final CallbackInfo ci) {
        if (!this.T) {
    ...
    private void labyMod$fireGameFpsLimitEvent(final InsertInfoReturnable<Integer> ci) {
        ...
        ci.setReturnValue((this.f == null && this.m != null) ? 60 : this.t.i);
    }
    ...
    private int labyMod$fireHotbarSlotChangeEvent() {
        ...
        final ClientHotbarSlotChangeEvent event = new ClientHotbarSlotChangeEvent(this.h.bv.d, slotDelta);
