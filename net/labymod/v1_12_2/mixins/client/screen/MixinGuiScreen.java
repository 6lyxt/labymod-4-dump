...
public abstract class MixinGuiScreen extends bir implements LabyScreenRendererAccessor, GuiScreenAccessor
{
    ...
    public boolean wrappedMouseClickMove(final MutableMouse mouse, final int button, final double deltaX, final double deltaY) {
        this.a(mouse.getX(), mouse.getY(), button, bib.I());
        return false;
    }
    ...
    public void labyMod$fireScreenInitEvent(final bib mc, final int width, final int height, final InsertInfo ci) {
        Laby.fireEvent(new VersionedScreenInitEvent(this));
    }
    
    @Insert(method = { "handleComponentClick" }, at = @At("HEAD"), cancellable = true)
    private void labyMod$handleCustomClickEvent(final hh component, final InsertInfoReturnable<Boolean> ci) {
        if (component == null) {
            return;
        }
        final hg clickEvent = component.b().h();
        if (!(clickEvent instanceof VersionedClickEvent) || s()) {
            return;
        }
    ...
    private void labyMod$disableBackground(final blk instance, final int x, final int y, final int width, final int height, final int color0, final int color1) {
    ...
    public void resize(final int width, final int height) {
        this.a(bib.z(), width, height);
    }
    
    public void setMinecraft(final bib minecraft) {
        this.j = minecraft;
    }
    ...
    @Inject(method = { "drawHoveringText(Ljava/util/List;II)V" }, at = { @At(value = "INVOKE", target = "Lnet/minecraft/client/renderer/GlStateManager;disableDepth()V", shift = At.Shift.AFTER) })
    private void labyMod$enableDepthTest(final List<String> lines, final int x, final int y, final CallbackInfo ci) {
        bus.k();
    }
    
    @Inject(method = { "drawHoveringText(Ljava/util/List;II)V" }, at = { @At(value = "INVOKE", target = "Ljava/util/List;size()I", shift = At.Shift.BEFORE, ordinal = 2) })
    ...
    public void renderComponentHoverEffect(final Stack stack, final Style style, final int x, final int y) {
        ...
        this.a((hh)component, x, y);
    }
    
    @Shadow
    protected abstract void a(final hh p0, final int p1, final int p2);
