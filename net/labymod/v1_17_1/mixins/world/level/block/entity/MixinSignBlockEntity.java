...
public abstract class MixinSignBlockEntity implements SignBlockEntity
{
    ...
    private void labyMod$fireTileEntityPostLoadEvent(final na tag, final InsertInfo ci) {
        Laby.fireEvent(new SignBlockEntityPostLoadEvent(this, (NBTTagCompound)tag));
    }
    
    @Override
    public float getRotationYaw() {
        final ciq entity = (ciq)this;
        final ckt state = entity.n();
        final bzp block = state.b();
        if (block instanceof cho) {
            final gl facing = (gl)state.c((clw)cho.d);
        ...
        if (block instanceof cgn) {
            final Integer rotation = (Integer)state.c((clw)cgn.d);
