...
public abstract class MixinClientPacketListener implements ClientPacketListener
{
    ...
    @Shadow
    public abstract void a(final os p0);
    ...
    private void labyMod$fireDimensionChangeEvent(final rv packet, final InsertInfo ci) {
        final ww from = this.g.aa().a();
        final ww to = packet.c().a();
    ...
    private void labyMod$handlePlayerInfo(final ro packet, final InsertInfo ci) {
        for (final ro.b entry : packet.b()) {
            switch (packet.c()) {
                case a: {
                    final ejm info = this.j.get(entry.a().getId());
                    if (info == null) {
                        continue;
                    }
    ...
    private void labyMod$handleClassicPvPBlocking(final sl packet, final InsertInfo insertInfo) {
        final int entityId = packet.c();
        final atg entity = this.g.a(entityId);
        if (!(entity instanceof bke)) {
            return;
        }
        final bke player = (bke)entity;
        final List<wi.a<?>> unpackedData = packet.b();
    ...
    public os labyMod$modifyReadson(final os component) {
        ...
        return (os)Laby.fireEvent(new ServerKickEvent(serverData, (Component)component, ServerKickEvent.Context.PLAY)).reason();
    }
    
    @Inject(method = { "send(Lnet/minecraft/network/protocol/game/ServerboundResourcePackPacket$Action;)V" }, at = { @At("HEAD") })
    private void labyMod$disableCustomFont(final us.a action, final CallbackInfo ci) {
        if (action == us.a.a) {
    ...
    private void labyMod$fireAbilityUpdateEvent(final rk packet, final CallbackInfo ci) {
        Laby.fireEvent(new ClientPlayerAbilitiesUpdateEvent((ClientPlayer)dvp.C().s));
    }
    ...
    public void simulateKick(final Component reason) {
        ...
        this.a((os)Laby.labyAPI().minecraft().componentMapper().toMinecraftComponent(reason));
    }
}
