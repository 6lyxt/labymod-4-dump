...
public abstract class MixinScreen implements FileDropHandler, ScreenAccessor
{
    ...
    public void fireScreenInitEvent(final dvp mc, final int width, final int height, final InsertInfo ci) {
    ...
    public void renderThemeBackground(final dql stack, final int param1, final InsertInfo ci) {
    ...
    private void renderThemeBackground(Stack stack, final InsertInfo ci) {
        ...
        stack = ((stack != null) ? stack : ((VanillaStackAccessor)new dql()).stack());
    ...
    private void labyMod$disableMenuBackground(final dql poseStack, final int vOffset, final InsertInfo ci) {
    ...
    public void renderComponentHoverEffect(final Stack stack, final Style style, final int x, final int y) {
        this.shadow$a((dql)stack.getProvider().getPoseStack(), (pc)style, x, y);
    }
    
    @Shadow
    protected abstract void shadow$a(final dql p0, @Nullable final pc p1, final int p2, final int p3);
    
    @Override
    public void setMinecraft(final dvp minecraft) {
        this.e = minecraft;
    }
    ...
    public List<Object> getWrappedWidgets() {
        ...
        for (final dxy child : this.o) {
            wrappedWidgets.add(this.asVersionedWidget(child));
        }
        return wrappedWidgets;
    }
    
    private VersionedWidget asVersionedWidget(final dxy child) {
        ...
        if (child instanceof final dxx container) {
            for (final dxy entry : container.j()) {
