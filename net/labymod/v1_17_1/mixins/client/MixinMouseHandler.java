...
public abstract class MixinMouseHandler implements MouseHandlerAccessor
{
    ...
    private void labyMod$handleMouseScroll(final long windowHandle, final double xOffset, final double yOffset, final InsertInfo callback) {
        final dvt options = dvp.C().l;
        final double scrollOffset = options.aa ? Math.signum(yOffset) : yOffset;
        final double delta = scrollOffset * options.O;
    ...
    private void redirectPlayerRotation(final emm player, final double x, final double y) {
    ...
    public void grabMouseNative() {
        dpg.a(this.a.aD().i(), 212995, this.a.aD().m() / 2.0, this.a.aD().n() / 2.0);
    }
    
    @Override
    public void ungrabMouseNative() {
        dpg.a(this.a.aD().i(), 212993, this.a.aD().m() / 2.0, this.a.aD().n() / 2.0);
    }
    
    @Redirect(method = { "onScroll" }, at = @At(value = "FIELD", ordinal = 5, target = "Lnet/minecraft/client/MouseHandler;accumulatedScroll:D"))
    private double labyMod$fireHotbarSlotChangeEvent(final dvq instance) {
        final double delta = this.p;
        if (delta == 0.0) {
            return 0.0;
        }
        final emm player = this.a.s;
        final int slotDelta = MathHelper.clamp((int)delta, -1, 1);
        final ClientHotbarSlotChangeEvent event = new ClientHotbarSlotChangeEvent(player.fk().k, slotDelta);
