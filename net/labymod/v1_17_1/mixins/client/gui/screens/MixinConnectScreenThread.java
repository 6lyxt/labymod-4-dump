...
public class MixinConnectScreenThread
{
    ...
    private Optional<ejs> labyMod$resolveAddress(final ejv resolver, final ejt address) {
    ...
    public os labyMod$fireServerKickEvent(final os reason) {
        final ServerData serverData = Laby.labyAPI().serverController().createServerData(dvp.C().E());
        if (serverData == null) {
            return reason;
        }
        return (os)Laby.fireEvent(new ServerKickEvent(ConnectableServerData.from(serverData), (Component)reason, ServerKickEvent.Context.PRE_LOGIN)).reason();
    }
}
