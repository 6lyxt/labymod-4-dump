...
public class MixinServerGamePacketListenerImpl
{
    ...
    private void labyMod$fireDisconnectEventPre(final os reason, final InsertInfo ci) {
        final GameProfile profile = (GameProfile)this.b.fj();
        Laby.fireEvent(new IntegratedServerPlayerDisconnectEvent(Phase.PRE, profile));
    }
    
    @Insert(method = { "onDisconnect" }, at = @At("TAIL"))
    private void labyMod$fireDisconnectEventPost(final os reason, final InsertInfo ci) {
        final GameProfile profile = (GameProfile)this.b.fj();
