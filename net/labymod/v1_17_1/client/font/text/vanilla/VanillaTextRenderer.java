...
package net.labymod.v1_17_1.client.font.text.vanilla;

import org.jetbrains.annotations.NotNull;
import java.util.ArrayList;
import java.util.Iterator;
import java.util.List;
import net.labymod.v1_17_1.client.font.DynamicStringRenderOutput;
...
public class VanillaTextRenderer extends AbstractTextRenderer
{
    ...
    public float width(final int codepoint, final Style style) {
        final dwa.f provider = ((StringSplitterAccessor)this.font().b()).getWidthProvider();
        if (style == null || style.isEmpty()) {
            return provider.getWidth(codepoint, pc.a);
        }
        return provider.getWidth(codepoint, (pc)style);
    }
    ...
    public float width(final VisualCharSequence text) {
        return (float)this.font().a((ags)text.castTo(ags.class));
    }
    ...
    public TextBuffer render(final Stack stack) {
        ...
        if (this.isAlphaAboveThreshold(color)) {
            ...
            if (!hasMultiBufferSource) {
                multiBufferSource = dvp.C().aE().b();
            }
            stringWidth = this.renderComponent(stack, (eni)multiBufferSource, component, x, y, color);
            if (!hasMultiBufferSource) {
                ((eni.a)multiBufferSource).b();
            }
        }
        else {
    ...
    public void beginBatch(final Stack stack) {
        if (stack.getMultiBufferSource() == null) {
            stack.multiBufferSource(dvp.C().aE().b());
    ...
    public void endBatch(final Stack stack) {
        if (this.bufferSourceAdded) {
            ...
            if (multiBufferSource instanceof final eni.a bufferSource) {
                bufferSource.b();
            }
    ...
    private float renderComponent(final Stack stack, final eni bufferSource, final RenderableComponent component, final float x, final float y, final int color) {
    ...
    public float getWidth(final RenderableComponent component) {
        ...
        float finalWidth = 0.0f;
        for (final FormattedRenderableComponent formattedRenderableComponent : formattedRenderableComponents) {
            final ags formattedText = this.composite(formattedRenderableComponent);
    ...
    private void renderFormattedComponent(final d modelViewMatrix, final DynamicStringRenderOutput output, final FormattedRenderableComponent component, final float x, final float y, final int color) {
        for (final RenderableComponent child : component.components()) {
            final ags sequence = MinecraftUtil.fromText(child.getText(), child.style());
    ...
    private void renderText(final DynamicStringRenderOutput output, final ags sequence, final float x, final float y, final int color, final boolean dropShadow, final d modelViewMatrix) {
        ...
        if (dropShadow) {
            ...
            sequence.accept((agt)output);
            copiedModelViewMatrix.a(dwl.d);
        }
