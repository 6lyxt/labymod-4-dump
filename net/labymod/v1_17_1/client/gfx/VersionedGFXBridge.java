...
import net.labymod.v1_17_1.client.gfx.buffer.VersionedBlaze3DBufferSource;
...
public class VersionedGFXBridge extends LWJGL3GFXBridge
{
    ...
    public void bindResourceLocation(final ResourceLocation location, final boolean blur, final boolean mipmap) {
        ...
        final exn textureManager = dvp.C().M();
        final exa texture = textureManager.b((ww)location.getMinecraftLocation());
    ...
    public void setCursorPosition(double x, double y) {
        ...
        GLFW.glfwSetCursorPos(dvp.C().aD().i(), Math.max(Math.min(x, window.getRawWidth()), 0.0), Math.max(Math.min(y, window.getRawHeight()), 0.0));
    }
