...
public class VersionedChatModifier implements InternalChatModifier
{
    ...
    public void deleteMessage(final ChatMessage message) {
        final VersionedChatComponent accessor = (VersionedChatComponent)dvp.C().k.d();
    ...
    public void editMessage(final ChatMessage message, final Component component) {
        ...
        final dvk<os> prevMessage = accessor.getMessages().get(index);
        final os mapped = (os)new VersionedChatMessageComponent(message, (os)this.componentMapper.toMinecraftComponent(component));
        accessor.getMessages().set(index, (dvk<os>)new dvk(prevMessage.b(), (Object)mapped, prevMessage.c()));
        final Predicate<dvk<ags>> predicate = this.formattedFilter(message);
        int first = -1;
    ...
    private Predicate<dvk<os>> messageFilter(final ChatMessage message) {
    ...
    private Predicate<dvk<ags>> formattedFilter(final ChatMessage message) {
