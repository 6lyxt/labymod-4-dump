...
public class VersionedTextureRepository extends DefaultAbstractTextureRepository implements TextureRepository
{
    ...
    public void onTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof final PathResourceLocation path) {
                final exn textureManager = this.getTextureManager();
                final ww minecraftLocation = resourceLocation.getMinecraftLocation();
    ...
    public void register(final ResourceLocation location, final Texture texture, @Nullable final Runnable finishHandler) {
        final exa minecraftTexture = this.convertToVersionedTexture(texture);
    ...
    public void registerTexture(final ResourceLocation location, final Object texture, final Runnable finishHandler) {
        if (texture instanceof final exa minecraftTexture) {
        ...
        throw new IllegalStateException(texture.getClass().getName() + " is not a subtype of " + exa.class.getName());
    }
    
    @Override
    public void preloadTexture(final ResourceLocation location) {
        this.registerTexture(location, (exa)new exi((ww)location.getMinecraftLocation()));
    }
    ...
    public Texture getTexture(final ResourceLocation location) {
        ...
        final exa texture = this.getTextureManager().b((ww)location.getMinecraftLocation());
    ...
    private void release(final ResourceLocation location, final Runnable releaseHandler) {
        this.getTextureManager().c((ww)location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull final ResourceLocation location, final exa texture) {
        this.getTextureManager().a((ww)location.getMinecraftLocation(), texture);
    }
    
    private void registerTexture(@NotNull final ResourceLocation location, final exa texture, final Runnable finishHandler) {
    ...
    private exa convertToVersionedTexture(final Texture texture) {
        if (texture instanceof final LabyTexture labyTexture) {
            return (exa)new VersionedLabyTexture(labyTexture);
        }
        if (texture instanceof final exa abstractTexture) {
            return abstractTexture;
        }
        throw this.createSubtypeException(texture, exa.class);
    }
}
