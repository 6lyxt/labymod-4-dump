...
package net.labymod.v1_20_2.client.multiplayer.server.storage;
...
public class VersionedServerList extends DefaultServerList
{
    ...
    public void load() {
        try {
            ...
            final qw root = rh.b(new File(this.gameDirectory, "servers.dat"));
            if (root == null) {
                return;
            }
            final rc list = root.c("servers", 10);
    ...
    private qw serverDataToTag(final StorageServerData data) {
        final qw tag = new qw();
        ...
        if (data.hasMetadata()) {
            ...
            tag.a("labyModMetadata", (rq)metaTag);
        }
        return tag;
    }
    
    private StorageServerData tagToServerData(final qw tag) {
        ...
        if (tag.b("labyModMetadata", 10)) {
            final qw metaTag = tag.p("labyModMetadata");
            for (final String key : metaTag.e()) {
                final rq metaValue = metaTag.c(key);
                String value;
                if (metaValue instanceof ro) {
                    value = metaValue.r_();
                }
                else {
                    if (!(metaValue instanceof qt)) {
                        continue;
                    }
                    final qt byteArrayTag = (qt)metaValue;
