...
package net.labymod.v1_20_2.client.resources.sound;
...
public class VersionedMinecraftSounds implements MinecraftSounds
{
    ...
    public void playChatFilterSound() {
        eqv.O().ai().a((gcm)gch.a((he)apd.qp, 1.0f));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch) {
        this.playSound(gch.a(new apc((aew)location.getMinecraftLocation(), 16.0f, false), pitch, volume));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch, final FloatVector3 position) {
        this.playSound(new gch((aew)location.getMinecraftLocation(), ape.a, volume, pitch, gcm.t(), false, 0, gcm.a.a, (double)position.getX(), (double)position.getY(), (double)position.getZ(), true));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch, final DoubleVector3 position) {
        this.playSound(new gch((aew)location.getMinecraftLocation(), ape.a, volume, pitch, gcm.t(), false, 0, gcm.a.a, position.getX(), position.getY(), position.getZ(), true));
    }
    
    private void playSound(final gch sound) {
        sound.a(eqv.O().ai());
        eqv.O().ai().a((gcm)sound);
    }
}
