...
public class VersionedChatExecutor extends DefaultChatExecutor
{
    ...
    public void chat(final String message, final boolean addToHistory) {
        this.dummyChatScreen.b(eqv.O(), 0, 0);
    ...
    public void displayActionBar(final Component message, final boolean animate) {
        final ese chatGui = eqv.O().l;
        final tl component = (tl)this.componentMapper.toMinecraftComponent(message);
    ...
    public void showTitle(@NotNull final Title title) {
        ...
        if (title.getTitle() != null) {
            eqv.O().l.c((tl)Laby.references().componentMapper().toMinecraftComponent(title.getTitle()));
        }
        else {
            eqv.O().l.c((tl)tl.h());
        }
        if (title.getSubTitle() != null) {
            eqv.O().l.b((tl)Laby.references().componentMapper().toMinecraftComponent(title.getSubTitle()));
        }
        eqv.O().l.a(title.getFadeInTicks(), title.getStayTicks(), title.getFadeOutTicks());
    }
