...
public class VersionedChatModifier implements InternalChatModifier
{
    ...
    public void fireChatClear(final boolean clearHistory) {
        eqv.O().l.d().a(clearHistory);
    }
    
    @Override
    public void deleteMessage(final ChatMessage message) {
        final VersionedChatComponent accessor = (VersionedChatComponent)eqv.O().l.d();
    ...
    public void editMessage(final ChatMessage message, final Component component) {
        ...
        final eqp prevMessage = accessor.getMessages().get(index);
        final tl mapped = (tl)new VersionedChatMessageComponent(message, (tl)this.componentMapper.toMinecraftComponent(component));
        accessor.getMessages().set(index, new eqp(prevMessage.a(), mapped, prevMessage.c(), prevMessage.d()));
        final Predicate<eqp.a> predicate = this.formattedFilter(message);
        int first = -1;
    ...
    private Predicate<eqp> messageFilter(final ChatMessage message) {
    ...
    private Predicate<eqp.a> formattedFilter(final ChatMessage message) {
