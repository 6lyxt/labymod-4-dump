...
public class ButtonConverter extends AbstractMinecraftWidgetConverter<esi, ButtonWidget>
{
    ...
    @Override
    public ButtonWidget createDefault(final esi source) {
        final ButtonWidget destination = ButtonWidget.component((source instanceof etu.b) ? null : this.mapComponent(source.m()), null, Pressable.NOOP);
    ...
    @Override
    public void update(final esi source, final ButtonWidget destination) {
    ...
    @Override
    public String getWidgetId(final esi source) {
        return this.componentMapper.getTranslationKeyOfComponent(source.m());
    }
    
    @Override
    public List<String> getWidgetIds(final esi source) {
        return this.componentMapper.getTranslationKeysOfComponent(source.m());
    }
    
    class ImageButtonMapper implements Mapper<esi, ButtonWidget>
    {
        @Override
        public ButtonWidget map(final esi source) {
            if (source instanceof final etc imageButton) {
                ...
                final Icon icon = Icon.sprite(location, accessor.getXTexStart(), accessor.getYTexStart(), source.l(), source.i(), accessor.getTextureWidth(), accessor.getTextureHeight());
    ...
    class LockIconButtonMapper implements Mapper<esi, ButtonWidget>
    {
        ...
        @Override
        public ButtonWidget map(final esi source) {
            if (source instanceof final etg lockIconButton) {
                final int size = source.l();
