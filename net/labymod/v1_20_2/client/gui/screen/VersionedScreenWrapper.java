...
package net.labymod.v1_20_2.client.gui.screen;
...
{
    private final eyk screen;
    private final esf graphics;
    private Metadata metadata;
    private boolean initialized;
    
    public VersionedScreenWrapper(final eyk screen) {
        if (screen == null) {
            throw new NullPointerException("screen is null");
        }
        final eqv minecraft = eqv.O();
        this.graphics = new esf(minecraft, minecraft.aO().b());
        this.screen = screen;
    ...
    public void initialize(final Parent parent) {
        if (parent != null) {
            this.screen.b(eqv.O(), (int)parent.bounds().getWidth(BoundsType.INNER), (int)parent.bounds().getHeight(BoundsType.INNER));
        }
        else {
            final Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(eqv.O(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void render(final Stack stack, final MutableMouse mouse, final float partialTicks) {
        ((GuiGraphicsAccessor)this.graphics).setPose((elp)stack.getProvider().getPoseStack());
    ...
    public void resize(final int width, final int height) {
        this.screen.b(eqv.O(), width, height);
        this.initialized = true;
    }
    
    @Override
    public void render(final ScreenContext context) {
        ((GuiGraphicsAccessor)this.graphics).setPose((elp)context.stack().getProvider().getPoseStack());
