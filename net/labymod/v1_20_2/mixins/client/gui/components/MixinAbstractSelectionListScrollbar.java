...
public abstract class MixinAbstractSelectionListScrollbar
{
    ...
    private int labyMod$render(final esl instance) {
        final int maxScroll = instance.m();
        if (maxScroll <= 0) {
            return 0;
        }
        ...
        final int x = this.c();
        final Bounds scrollBar = new PositionedBounds((float)x, (float)this.g, (float)(x + 6), (float)this.h);
        final float scrollBarHeight = scrollBar.getHeight();
        final int contentHeight = this.a() - this.l;
        final int scrollButtonHeight = MathHelper.ceilOrFloor(scrollBarHeight * scrollBarHeight / contentHeight);
        int scrolled = (int)this.l() * ((int)scrollBarHeight - scrollButtonHeight) / maxScroll;
        if (scrolled < 0) {
            scrolled = 0;
        }
