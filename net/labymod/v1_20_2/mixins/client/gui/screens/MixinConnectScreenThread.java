...
public class MixinConnectScreenThread
{
    ...
    private Optional<fkh> labyMod$resolveAddress(final fkk resolver, final fki address) {
        final InetSocketAddress resolved = ConnectAddressResolver.resolve(address.a(), address.b(), (AddressResolveCallback)this.d);
        return (resolved == null) ? Optional.empty() : Optional.of(fkh.a(resolved));
    }
    ...
    public tl labyMod$fireServerKickEvent(final tl reason) {
        ...
        return (tl)componentMapper.toMinecraftComponent(Laby.fireEvent(new ServerKickEvent(ConnectableServerData.from(data), componentMapper.fromMinecraftComponent(reason), ServerKickEvent.Context.PRE_LOGIN)).reason());
    }
}
