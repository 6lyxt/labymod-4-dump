...
public abstract class MixinScreen implements FileDropHandler, ScreenAccessor
{
    ...
    public void fireScreenInitEvent(final fmg mc, final int width, final int height, final InsertInfo ci) {
        Laby.fireEvent(new VersionedScreenInitEvent(this));
    }
    
    @Insert(method = { "renderBackground" }, at = @At("HEAD"), cancellable = true)
    public void renderThemeBackground(final fns graphics, final int mouseX, final int mouseY, final float partialTicks, final InsertInfo ci) {
    ...
    private void renderThemeBackground(Stack stack, final InsertInfo ci) {
        ...
        stack = ((stack != null) ? stack : ((VanillaStackAccessor)new fgs()).stack());
    ...
    private void labyMod$hideMenuBackground(final fns $$0, final CallbackInfo ci) {
    ...
    private void labyMod$disableMenuBackground(final fns graphics, final int mouseX, final int mouseY, final float partialTicks, final InsertInfo ci) {
        if (Laby.labyAPI().config().appearance().hideMenuBackground().get()) {
            ci.cancel();
        }
    }
    ...
    public void renderComponentHoverEffect(final Stack stack, final Style style, final int x, final int y) {
        if (this.labyMod$graphics == null) {
            this.labyMod$graphics = new fns(this.m, this.m.aR().c());
        }
        ((GuiGraphicsAccessor)this.labyMod$graphics).setPose((fgs)stack.getProvider().getPoseStack());
        this.labyMod$graphics.a(this.p, (ys)style, x, y);
    }
    
    @Override
    public void setMinecraft(final fmg minecraft) {
        this.m = minecraft;
    }
    ...
    public List<Object> getWrappedWidgets() {
        ...
        for (final fqd child : this.d) {
            wrappedWidgets.add(this.asVersionedWidget(child));
        }
        return wrappedWidgets;
    }
    
    private VersionedWidget asVersionedWidget(final fqd child) {
        ...
        if (child instanceof final fqc container) {
            for (final fqd entry : container.aI_()) {
