...
public abstract class MixinGameRenderer
{
    ...
    private Matrix4f labyMod$setViewMatrix(Matrix4f self, final Quaternionfc quaternion, final Operation<Matrix4f> original, final flw deltaTracker) {
    ...
    private void labyMod$firePreRenderHandEvent(final flp camera, final float partialTicks, final Matrix4f viewMatrix, final CallbackInfo ci) {
    ...
    private void labyMod$firePostRenderHandEvent(final flp camera, final float partialTicks, final Matrix4f viewMatrix, final CallbackInfo ci) {
        final fgs poseStack = MinecraftUtil.levelRenderContext().getPoseStack();
    ...
    public void labyMod$fireFieldOfViewTickEvent(final InsertInfo ci) {
        float modifier = 1.0f;
        final bvk ao = this.i.ao();
        if (ao instanceof final gke cameraEntity) {
    ...
    private boolean labyMod$noViewBobbing(final glb instance, final fgs stack, final float partialTicks) {
