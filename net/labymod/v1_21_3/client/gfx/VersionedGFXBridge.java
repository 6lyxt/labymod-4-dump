...
import net.labymod.v1_21_3.client.gfx.buffer.VersionedBlaze3DBufferSource;
...
public class VersionedGFXBridge extends LWJGL3GFXBridge
{
    ...
    public void bindResourceLocation(final ResourceLocation location, final boolean blur, final boolean mipmap) {
        ...
        final hbm textureManager = fmg.Q().aa();
        final haw texture = textureManager.a((alz)location.getMinecraftLocation());
    ...
    public void setCursorPosition(double x, double y) {
        ...
        GLFW.glfwSetCursorPos(fmg.Q().aO().h(), Math.max(Math.min(x, window.getRawWidth()), 0.0), Math.max(Math.min(y, window.getRawHeight()), 0.0));
    }
