...
public class VersionedMinecraftSounds implements MinecraftSounds
{
    ...
    public void playChatFilterSound() {
        fmg.Q().ak().a((hfi)hfd.a((jq)axf.sq, 1.0f));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch) {
        this.playSound(hfd.a(axe.a((alz)location.getMinecraftLocation(), 16.0f), pitch, volume));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch, final FloatVector3 position) {
        this.playSound(new hfd((alz)location.getMinecraftLocation(), axg.a, volume, pitch, hfi.t(), false, 0, hfi.a.a, (double)position.getX(), (double)position.getY(), (double)position.getZ(), true));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch, final DoubleVector3 position) {
        this.playSound(new hfd((alz)location.getMinecraftLocation(), axg.a, volume, pitch, hfi.t(), false, 0, hfi.a.a, position.getX(), position.getY(), position.getZ(), true));
    }
    
    private void playSound(final hfd sound) {
        sound.a(fmg.Q().ak());
        fmg.Q().ak().a((hfi)sound);
    }
}
