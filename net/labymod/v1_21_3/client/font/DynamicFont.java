...
public class DynamicFont extends fnq implements FontHeightAccessor
{
    ...
    private final fnq delegate;
    private final fmq customSplitter;
    
    public DynamicFont(final fnq delegate) {
        super(delegate.f, delegate.g);
        this.delegate = delegate;
        this.customSplitter = new fmq((codepoint, style) -> DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().width(codepoint, (Style)style));
    }
    
    @NotNull
    public fqy a(@NotNull final alz location) {
    ...
    public int a(final String text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final Matrix4f modelViewMatrix, @NotNull final gll bufferSource, @NotNull final fnq.a displayMode, final int backgroundColor, final int packedLightCoords, final boolean bidirectionalShaping) {
    ...
    public int a(@NotNull final azq text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final Matrix4f modelViewMatrix, @NotNull final gll bufferSource, @NotNull final fnq.a displayMode, final int backgroundColor, final int packedLightCoords, final boolean inverseDepth) {
        if (this.isForceVanillaFont()) {
    ...
    public void a(@NotNull final azq text, final float x, final float y, final int color, final int outlineColor, @NotNull final Matrix4f modelViewMatrix, @NotNull final gll bufferSource, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
        ...
        builder.pos(x, y).text(labyComponent).drawMode(MinecraftUtil.fromMinecraft(fnq.a.c)).color(color).shadow(false).render(stack);
        stack.pop();
    }
    ...
    public int a(@NotNull final ya text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
    ...
    public int a(@NotNull final azq text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
        final StringOutput stringOutput = new StringOutput();
        text.accept((azr)stringOutput);
        return this.a((ya)DynamicFont.COMPONENT_MAPPER.toMinecraftComponent(stringOutput.getLabyComponent()));
    }
    ...
    public String a(@NotNull final String text, final int maxWidth, final boolean tail) {
        ...
        if (tail) {
            return this.customSplitter.c(text, maxWidth, ys.a);
        }
        return this.customSplitter.b(text, maxWidth, ys.a);
    }
    ...
    @NotNull
    public ya a(@NotNull final ya text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text, maxWidth);
        }
        return this.customSplitter.a(text, maxWidth, ys.a);
    }
    ...
    public int b(@NotNull final String text, final int maxWidth) {
        ...
        final int lines = this.customSplitter.g(text, maxWidth, ys.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    public int b(@NotNull final ya text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.b(text, maxWidth);
        }
        final int lines = this.customSplitter.b(text, maxWidth, ys.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    @NotNull
    public List<azq> c(@NotNull final ya text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.c(text, maxWidth);
        }
        return us.a().a(this.customSplitter.b(text, maxWidth, ys.a));
    }
    ...
    @Deprecated
    private void endBatch(final gll bufferSource) {
        if (bufferSource instanceof final gll.a source) {
            source.a();
        }
    }
    ...
    static class StringOutput implements azr
    {
        ...
        public boolean accept(final int position, @NotNull final ys style, final int codepoint) {
