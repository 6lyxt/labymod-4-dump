...
package net.labymod.v1_21_3.client.font.text.vanilla;
...
import net.labymod.v1_21_3.client.font.DynamicStringRenderOutput;
...
public class VanillaTextRenderer extends AbstractTextRenderer
{
    ...
    public float width(final int codepoint, final Style style) {
        final fmq.f provider = ((StringSplitterAccessor)this.font().b()).getWidthProvider();
        if (style == null || style.isEmpty()) {
            return provider.getWidth(codepoint, ys.a);
        }
        return provider.getWidth(codepoint, (ys)style);
    }
    ...
    public float width(final VisualCharSequence text) {
        return (float)this.font().a((azq)text.castTo(azq.class));
    }
    ...
    public TextBuffer render(final Stack stack) {
        ...
        if (this.isAlphaAboveThreshold(color)) {
            ...
            if (!hasMultiBufferSource) {
                multiBufferSource = fmg.Q().aR().c();
            }
            stringWidth = this.renderComponent(stack, (gll)multiBufferSource, component, x, y, color);
            if (!hasMultiBufferSource) {
                ((gll.a)multiBufferSource).b();
            }
        }
        else {
    ...
    public void beginBatch(final Stack stack) {
        if (stack.getMultiBufferSource() == null) {
            stack.multiBufferSource(fmg.Q().aR().c());
    ...
    public void endBatch(final Stack stack) {
        if (this.bufferSourceAdded) {
            ...
            if (multiBufferSource instanceof final gll.a bufferSource) {
                bufferSource.b();
            }
    ...
    private float renderComponent(final Stack stack, final gll bufferSource, final RenderableComponent component, final float x, final float y, final int color) {
        ...
        for (final FormattedRenderableComponent formattedRenderableComponent : formattedRenderableComponents) {
            final DynamicStringRenderOutput renderOutput = new DynamicStringRenderOutput(font, bufferSource, x, y, color, this.backgroundColor, this.shadow, MathHelper.mapper().toMatrix4f(stack.getProvider().getPosition()), MinecraftUtil.toMinecraft(this.textDrawMode), VanillaTextRenderer.RENDER_ENVIRONMENT_CONTEXT.getPackedLightWithCondition(), true);
            ...
            final float renderedWidth = renderOutput.a(x);
    ...
    public float getWidth(final RenderableComponent component) {
        ...
        float finalWidth = 0.0f;
        for (final FormattedRenderableComponent formattedRenderableComponent : formattedRenderableComponents) {
            final azq formattedText = this.composite(formattedRenderableComponent);
    ...
    private void renderFormattedComponent(final Matrix4f modelViewMatrix, final DynamicStringRenderOutput output, final FormattedRenderableComponent component, final float x, final float y, final int color) {
        for (final RenderableComponent child : component.components()) {
            final azq sequence = MinecraftUtil.fromText(child.getText(), child.style());
    ...
    private void renderText(final DynamicStringRenderOutput output, final azq sequence, final float x, final float y, final int color, final boolean dropShadow, final Matrix4f modelViewMatrix) {
        ...
        if (dropShadow) {
            ...
            sequence.accept((azr)output);
            copiedModelViewMatrix.translate((Vector3fc)fnq.e);
        }
