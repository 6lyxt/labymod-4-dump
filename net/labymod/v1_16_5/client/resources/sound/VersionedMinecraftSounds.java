...
public class VersionedMinecraftSounds implements MinecraftSounds
{
    ...
    public void playChatFilterSound() {
        djz.C().W().a((emt)emp.a(adq.jv, 1.0f));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch) {
        this.playSound(emp.a(new adp((vk)location.getMinecraftLocation()), pitch, volume));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch, final FloatVector3 position) {
        this.playSound(new emp((vk)location.getMinecraftLocation(), adr.a, volume, pitch, false, 0, emt.a.a, (double)position.getX(), (double)position.getY(), (double)position.getZ(), true));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch, final DoubleVector3 position) {
        this.playSound(new emp((vk)location.getMinecraftLocation(), adr.a, volume, pitch, false, 0, emt.a.a, position.getX(), position.getY(), position.getZ(), true));
    }
    
    private void playSound(final emp sound) {
        sound.a(djz.C().W());
        djz.C().W().a((emt)sound);
    }
}
