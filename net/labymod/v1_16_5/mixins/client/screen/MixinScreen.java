...
public abstract class MixinScreen implements FileDropHandler, ScreenAccessor
{
    ...
    public void fireScreenInitEvent(final djz mc, final int width, final int height, final InsertInfo ci) {
    ...
    public void renderThemeBackground(final dfm stack, final int param1, final InsertInfo ci) {
    ...
    private void renderThemeBackground(Stack stack, final InsertInfo ci) {
        ...
        stack = ((stack != null) ? stack : ((VanillaStackAccessor)new dfm()).stack());
    ...
    private void labyMod$disableMenuBackground(final dfm poseStack, final int vOffset, final InsertInfo ci) {
    ...
    public void renderComponentHoverEffect(final Stack stack, final Style style, final int x, final int y) {
        this.shadow$a((dfm)stack.getProvider().getPoseStack(), (ob)style, x, y);
    }
    
    @Shadow
    protected abstract void shadow$a(final dfm p0, @Nullable final ob p1, final int p2, final int p3);
    
    @Override
    public void setMinecraft(final djz minecraft) {
        this.i = minecraft;
    }
    ...
    public List<Object> getWrappedWidgets() {
        ...
        for (final dmi child : this.e) {
            wrappedWidgets.add(this.asVersionedWidget(child));
        }
        return wrappedWidgets;
    }
    
    private VersionedWidget asVersionedWidget(final dmi child) {
        ...
        if (child instanceof final dmh container) {
            for (final dmi entry : container.au_()) {
