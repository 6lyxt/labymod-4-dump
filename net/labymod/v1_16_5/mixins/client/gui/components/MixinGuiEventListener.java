...
public interface MixinGuiEventListener
{
    default boolean handleConvertedWidget(final BiFunction<AbstractWidgetConverter, AbstractWidget<?>, Boolean> converterFunction) {
        ...
        final WidgetWatcher<?> watcher = ((ConvertableMinecraftWidget)this).getWatcher();
    ...
    default boolean a(final double mouseX, final double mouseY, final int button) {
        return this.handleConvertedWidget((converter, widget) -> Laby.labyAPI().minecraft().updateMouse(mouseX, mouseY, mouse -> {
    ...
    @Overwrite
    default boolean c(final double mouseX, final double mouseY, final int button) {
        return this.handleConvertedWidget((converter, widget) -> Laby.labyAPI().minecraft().updateMouse(mouseX, mouseY, mouse -> {
            final MouseButton mouseButton = DefaultKeyMapper.pressMouse(button);
    ...
    default boolean a(final double mouseX, final double mouseY, final double delta) {
        return this.handleConvertedWidget((converter, widget) -> Laby.labyAPI().minecraft().updateMouse(mouseX, mouseY, mouse -> converter.mouseScrolled(widget, mouse, delta)));
    }
