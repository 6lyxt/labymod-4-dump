...
public class MixinConnectScreenThread
{
    ...
    private Optional<fzm> labyMod$resolveAddress(final fzp resolver, final fzn address) {
        final InetSocketAddress resolved = ConnectAddressResolver.resolve(address.a(), address.b(), (AddressResolveCallback)this.e);
        return (resolved == null) ? Optional.empty() : Optional.of(fzm.a(resolved));
    }
    ...
    public xp labyMod$fireServerKickEvent(final xp reason) {
        ...
        return (xp)componentMapper.toMinecraftComponent(Laby.fireEvent(new ServerKickEvent(ConnectableServerData.from(data), componentMapper.fromMinecraftComponent(reason), ServerKickEvent.Context.PRE_LOGIN)).reason());
    }
}
