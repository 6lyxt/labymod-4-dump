...
import net.labymod.v1_20_5.client.overlay.CustomLoadingOverlay;
...
public abstract class MixinMinecraft
{
    ...
    private String labyMod$censorSessionId(final ffu instance) {
        return "********************************";
    }
    
    @Redirect(method = { "<init>" }, at = @At(value = "FIELD", target = "Lnet/minecraft/client/main/GameConfig$UserData;proxy:Ljava/net/Proxy;"))
    private Proxy labyMod$callGameInitializeResource(final fsr.d userData) {
    ...
    private File labyMod$onPreGameStarted(final fsr.a instance) {
    ...
    private ezd labyMod$adjustWindowSize(final geh instance, eyq displayData, final String $$1, final String $$2) {
        ...
        if (sizePair != null) {
            displayData = new eyq((int)sizePair.getFirst(), (int)sizePair.getSecond(), displayData.c, displayData.d, displayData.e);
        }
    ...
    private void labyMod$maximizeDisplay(final fsr $$0, final CallbackInfo ci) {
    ...
    private void labyMod$firePostGameStartedInitializeEvent(final fsr param0, final CallbackInfo ci) {
    ...
    private String labyMod$modifiedTitle(final StringBuilder stringBuilder) {
        ...
        final fxx listener = this.L();
        if (listener != null && listener.k().i()) {
            extraBuilder.append(" - ");
            final fyk currentServer = this.S();
            if (this.aP != null && !this.aP.r()) {
                extraBuilder.append(gqg.a("title.singleplayer", new Object[0]));
            }
            else if (currentServer != null && currentServer.e()) {
                extraBuilder.append(gqg.a("title.multiplayer.realms", new Object[0]));
            }
            else if (this.aP == null && (currentServer == null || !currentServer.d())) {
                extraBuilder.append(gqg.a("title.multiplayer.other", new Object[0]));
            }
            else {
                extraBuilder.append(gqg.a("title.multiplayer.lan", new Object[0]));
            }
        }
        return LabyMod.getInstance().createTitle("1.20.5", extraBuilder.toString());
    }
    
    @Redirect(method = { "setScreen" }, at = @At(value = "INVOKE", target = "Lnet/minecraft/client/gui/screens/Screen;removed()V"))
    private void labyMod$cancelRemovedScreen(final fne screen) {
    }
    ...
    private fne labyMod$fireScreenOpenEvent(fne newScreen) {
        ...
        try {
            ...
            newScreen = ((instance == null) ? null : ((fne)instance.wrap().getVersionedScreen()));
        }
    ...
    private ScreenWrapper labyMod$createScreenWrapper(final fne screen) {
    ...
    private fmq labyMod$createLoadingOverlay(final ffg minecraft, final aul reloadInstance, final Consumer onFinish, final boolean fadeIn) {
        ...
        return (type == FadeOutAnimationType.FADING && !DynamicBackgroundController.isEnabled()) ? new fmq(minecraft, reloadInstance, onFinish, fadeIn) : new CustomLoadingOverlay(minecraft, reloadInstance, onFinish, fadeIn);
    }
    
    @Insert(method = { "disconnect(Lnet/minecraft/client/gui/screens/Screen;Z)V" }, at = @At("HEAD"))
    private void labyMod$firePreNetworkDisconnectEvent(final fne screen, final boolean transferring, final InsertInfo ci) {
    ...
    private void labyMod$firePostNetworkDisconnectEvent(final fne screen, final boolean transferring, final InsertInfo ci) {
        Laby.labyAPI().serverController().disconnect(Phase.POST);
    }
    
    @Insert(method = { "setLevel" }, at = @At("HEAD"))
    private void labyMod$firePreWorldLoadEvent(final fxw level, final fnc.a reason, final InsertInfo ci) {
        if (level != null) {
    ...
    private void labyMod$firePostWorldLoadEvent(final fxw level, final fnc.a reason, final InsertInfo ci) {
        if (level != null) {
    ...
    private void labyMod$fireGameRenderEvent(final gdi gameRenderer, final float partialTicks, final long nanoTime, final boolean running) {
    ...
    @Overwrite
    public static void a(final ffg mc, final File file, final o report) {
