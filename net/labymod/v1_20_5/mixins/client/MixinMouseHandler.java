...
public abstract class MixinMouseHandler implements MouseHandlerAccessor
{
    ...
    private void redirectPlayerRotation(final gcr player, final double x, final double y) {
    ...
    public void grabMouseNative() {
        eyu.a(this.a.aO().i(), 212995, this.a.aO().m() / 2.0, this.a.aO().n() / 2.0);
    }
    
    @Override
    public void ungrabMouseNative() {
        eyu.a(this.a.aO().i(), 212993, this.a.aO().m() / 2.0, this.a.aO().n() / 2.0);
    }
    
    @Redirect(method = { "onScroll" }, at = @At(value = "FIELD", ordinal = 5, target = "Lnet/minecraft/client/MouseHandler;accumulatedScrollY:D"))
    private double labyMod$fireHotbarSlotChangeEvent(final ffh instance) {
        final double delta = this.q;
        if (delta == 0.0) {
            return 0.0;
        }
        final gcr player = this.a.s;
        final int slotDelta = MathHelper.clamp((int)delta, -1, 1);
        final ClientHotbarSlotChangeEvent event = new ClientHotbarSlotChangeEvent(player.gc().k, slotDelta);
