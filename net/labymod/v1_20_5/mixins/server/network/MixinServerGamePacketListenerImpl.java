...
package net.labymod.v1_20_5.mixins.server.network;
...
public class MixinServerGamePacketListenerImpl
{
    ...
    private void labyMod$fireDisconnectEventPre(final xp reason, final InsertInfo ci) {
        final GameProfile profile = (GameProfile)this.f.gb();
        Laby.fireEvent(new IntegratedServerPlayerDisconnectEvent(Phase.PRE, profile));
    }
    
    @Insert(method = { "onDisconnect" }, at = @At("TAIL"))
    private void labyMod$fireDisconnectEventPost(final xp reason, final InsertInfo ci) {
        final GameProfile profile = (GameProfile)this.f.gb();
