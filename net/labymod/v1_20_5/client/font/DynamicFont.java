...
package net.labymod.v1_20_5.client.font;
...
public class DynamicFont extends fgq implements FontHeightAccessor
{
    ...
    private final fgq delegate;
    private final ffr customSplitter;
    
    public DynamicFont(final fgq delegate) {
        super(delegate.f, delegate.g);
        this.delegate = delegate;
        this.customSplitter = new ffr((codepoint, style) -> DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().width(codepoint, (Style)style));
    }
    
    @NotNull
    public fjw a(@NotNull final alf location) {
    ...
    public int b(final String text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final Matrix4f modelViewMatrix, @NotNull final gdp bufferSource, @NotNull final fgq.a displayMode, final int backgroundColor, final int packedLightCoords, final boolean bidirectionalShaping) {
    ...
    public int b(@NotNull final ayl text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final Matrix4f modelViewMatrix, @NotNull final gdp bufferSource, @NotNull final fgq.a displayMode, final int backgroundColor, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
    ...
    public void a(@NotNull final ayl text, final float x, final float y, final int color, final int outlineColor, @NotNull final Matrix4f modelViewMatrix, @NotNull final gdp bufferSource, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
        ...
        builder.pos(x, y).text(labyComponent).drawMode(MinecraftUtil.fromMinecraft(fgq.a.c)).color(color).shadow(false).render(stack);
        stack.pop();
    }
    ...
    public int a(@NotNull final xu text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
    ...
    public int a(@NotNull final ayl text) {
        ...
        return this.a((xu)DynamicFont.COMPONENT_MAPPER.toMinecraftComponent(stringOutput.getLabyComponent()));
    }
    ...
    public String a(@NotNull final String text, final int maxWidth, final boolean tail) {
        ...
        if (tail) {
            return this.customSplitter.c(text, maxWidth, ym.a);
        }
        return this.customSplitter.b(text, maxWidth, ym.a);
    }
    ...
    @NotNull
    public xu a(@NotNull final xu text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text, maxWidth);
        }
        return this.customSplitter.a(text, maxWidth, ym.a);
    }
    ...
    public int b(@NotNull final String text, final int maxWidth) {
        ...
        final int lines = this.customSplitter.g(text, maxWidth, ym.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    public int b(@NotNull final xu text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.b(text, maxWidth);
        }
        final int lines = this.customSplitter.b(text, maxWidth, ym.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    @NotNull
    public List<ayl> c(@NotNull final xu text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.c(text, maxWidth);
        }
        return un.a().a(this.customSplitter.b(text, maxWidth, ym.a));
    }
    ...
    static class StringOutput implements aym
    {
        ...
        public boolean accept(final int position, @NotNull final ym style, final int codepoint) {
