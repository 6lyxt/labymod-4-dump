...
package net.labymod.v1_20_5.client.font.text.vanilla;
...
import net.labymod.v1_20_5.client.font.DynamicStringRenderOutput;
...
public class VanillaTextRenderer extends AbstractTextRenderer
{
    ...
    public float width(final int codepoint, final Style style) {
        final ffr.f provider = ((StringSplitterAccessor)this.font().b()).getWidthProvider();
        if (style == null || style.isEmpty()) {
            return provider.getWidth(codepoint, ym.a);
        }
        return provider.getWidth(codepoint, (ym)style);
    }
    ...
    public TextBuffer render(final Stack stack) {
        ...
        if (this.isAlphaAboveThreshold(color)) {
            ...
            if (!hasMultiBufferSource) {
                multiBufferSource = ffg.Q().aQ().c();
            }
            stringWidth = this.renderComponent(stack, (gdp)multiBufferSource, component, x, y, color);
            if (!hasMultiBufferSource) {
                ((gdp.a)multiBufferSource).b();
            }
        }
        else {
    ...
    public void beginBatch(final Stack stack) {
        if (stack.getMultiBufferSource() == null) {
            stack.multiBufferSource(ffg.Q().aQ().c());
    ...
    public void endBatch(final Stack stack) {
        if (this.bufferSourceAdded) {
            ...
            if (multiBufferSource instanceof final gdp.a bufferSource) {
                bufferSource.b();
            }
    ...
    private float renderComponent(final Stack stack, final gdp bufferSource, final RenderableComponent component, final float x, final float y, final int color) {
    ...
    private void renderText(final DynamicStringRenderOutput output, final ayl sequence, final float x, final float y, final int color, final boolean dropShadow, final Matrix4f modelViewMatrix) {
        ...
        if (dropShadow) {
            ...
            copiedModelViewMatrix.translate((Vector3fc)fgq.e);
        }
