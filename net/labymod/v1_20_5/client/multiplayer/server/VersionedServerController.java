...
import net.labymod.v1_20_5.client.multiplayer.server.storage.VersionedServerList;
...
public class VersionedServerController extends DefaultAbstractServerController
{
    ...
    public ServerData createServerData(final Object mojangServerData) {
        ...
        if (!(mojangServerData instanceof fyk)) {
            throw new IllegalArgumentException(String.format(Locale.ROOT, "%s is not an instance of %s!", mojangServerData.getClass().getName(), fyk.class.getName()));
        }
        final fyk serverData = (fyk)mojangServerData;
    ...
    private void sendCustomPayload(@NotNull final fxx connection, @NotNull final ResourceLocation identifier, final byte[] payload) {
    ...
    public void leaveServer() {
        ...
        final fyk currentServer = minecraft.S();
        final boolean realms = currentServer != null && currentServer.e();
        minecraft.r.Y();
        if (singleplayer) {
            minecraft.b((fne)new fmk((xp)this.componentMapper.toMinecraftComponent(Component.translatable("menu.savingLevel", new Component[0]))));
        }
        else {
            minecraft.y();
        }
        final fnj titleScreen = new fnj();
        fne display;
        if (singleplayer) {
            display = (fne)titleScreen;
        }
        else if (realms) {
            display = (fne)new fak((fne)titleScreen);
        }
        else {
            display = (fne)new fqc((fne)titleScreen);
        }
        minecraft.a(display);
    }
    ...
    public void joinServer(@NotNull final ConnectableServerData connectableServerData, @Nullable final Runnable loginExecutor) {
        ...
        flx.a(ffg.Q().y, ffg.Q(), fzn.a(serverData.b), (fyk)serverData, false, (fyo)null);
        if (ffg.Q().y instanceof fmf) {
    ...
    protected String getDefaultServerName() {
        return gqg.a("selectServer.defaultName", new Object[0]);
    }
    
    @Override
    public ScreenWrapper createEditServerScreen(final StorageServerData data, final Consumer<StorageServerData> callback) {
        final fne prevScreen = ffg.Q().y;
        final fyk mcData = new fyk(data.getName(), data.address().toString(), MinecraftUtil.toMinecraft(data.type()));
        mcData.a(fyk.a.values()[data.resourcePackStatus().ordinal()]);
        final fmg screen = new fmg(prevScreen, addServer -> {
            if (addServer) {
