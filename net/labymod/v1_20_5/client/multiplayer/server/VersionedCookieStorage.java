...
{
    private final fyo transferState;
    
    public VersionedCookieStorage(final fyo transferState) {
    ...
    public Map<ResourceLocation, byte[]> cookies() {
        final Map<alf, byte[]> vanillaCookies = this.transferState.a();
        final Map<ResourceLocation, byte[]> cookies = new HashMap<ResourceLocation, byte[]>(vanillaCookies.size());
        for (final Map.Entry<alf, byte[]> entry : vanillaCookies.entrySet()) {
