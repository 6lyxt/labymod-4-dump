...
package net.labymod.v1_20_5.client.gui.screen;
...
{
    private final fne screen;
    private final fgs graphics;
    private Metadata metadata;
    private boolean initialized;
    
    public VersionedScreenWrapper(final fne screen) {
        if (screen == null) {
            throw new NullPointerException("screen is null");
        }
        final ffg minecraft = ffg.Q();
        this.graphics = new fgs(minecraft, minecraft.aQ().c());
        this.screen = screen;
    ...
    public void initialize(final Parent parent) {
        if (parent != null) {
            this.screen.b(ffg.Q(), (int)parent.bounds().getWidth(BoundsType.INNER), (int)parent.bounds().getHeight(BoundsType.INNER));
        }
        else {
            final Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(ffg.Q(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void render(final Stack stack, final MutableMouse mouse, final float partialTicks) {
        ((GuiGraphicsAccessor)this.graphics).setPose((ezz)stack.getProvider().getPoseStack());
    ...
    public void resize(final int width, final int height) {
        this.screen.b(ffg.Q(), width, height);
        this.initialized = true;
    }
    
    @Override
    public void render(final ScreenContext context) {
        ((GuiGraphicsAccessor)this.graphics).setPose((ezz)context.stack().getProvider().getPoseStack());
