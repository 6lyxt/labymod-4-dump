...
public class LabyBiomeBlender implements dbc
{
    ...
    public LabyBiomeBlender setLevel(final dbz level) {
        this.level = level;
        return this;
    }
    ...
    public float a(@NotNull final je direction, final boolean shade) {
    ...
    @Nullable
    public dpi c_(@NotNull final iz blockPos) {
        return this.level.c_(blockPos);
    }
    
    @NotNull
    public dsd a_(@NotNull final iz blockPos) {
        return this.level.a_(blockPos);
    }
    
    @NotNull
    public enw b_(@NotNull final iz blockPos) {
    ...
    public int a(@NotNull final iz blockPos, @NotNull final dbk resolver) {
        final int x = blockPos.u();
        final int y = blockPos.v();
        final int z = blockPos.w();
        final int[] rgb = new int[3];
        int total = 0;
        final iz.a currentPos = new iz.a();
    ...
    private int processZAxis(final int x, final int y, final int z, final iz.a currentPos, final dbk resolver, int total, final int[] rgb, final int xIndex) {
    ...
    private int blendColors(final iz.a currentPos, final dbk resolver, int total, final int[] rgb) {
        final ji<dcy> holder = (ji<dcy>)this.level.t((iz)currentPos);
        final dcy biome = (holder == null) ? null : ((dcy)holder.a());
        if (biome != null) {
