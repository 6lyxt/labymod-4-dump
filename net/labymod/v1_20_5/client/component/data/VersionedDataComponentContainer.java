...
package net.labymod.v1_20_5.client.component.data;
...
public class VersionedDataComponentContainer implements DataComponentContainer
{
    ...
    public Set<DataComponentKey> keySet() {
        ...
        for (final kl<?> dataComponentType : wrapped.b()) {
            final alf key = this.findId(dataComponentType);
            if (key == null) {
    ...
    public <T> T get(final DataComponentKey key) {
        final alf resourceLocation = alf.a(key.name());
        if (resourceLocation == null) {
            return null;
        }
        final kl<?> dataComponentType = this.findType(resourceLocation);
        if (dataComponentType == null) {
            return null;
        }
        final ki wrapped = this.getWrapped();
        final ko<?> typedDataComponent = (ko<?>)wrapped.c((kl)dataComponentType);
        if (typedDataComponent == null) {
            return null;
        }
        final DataResult<vp> result = (DataResult<vp>)typedDataComponent.a((DynamicOps)vg.a);
    ...
    public <T> void set(final DataComponentKey key, final T value) {
        ...
        final kn patchedDataComponentMap = (kn)wrapped;
        final alf resourceLocation = alf.a(key.name());
        if (resourceLocation == null) {
            return;
        }
        final kl type = this.findType(resourceLocation);
        if (type == null) {
            return;
        }
        final DataResult parsedResult = type.c().decode((DynamicOps)vg.a, (Object)value);
    ...
    public Iterator<TypedDataComponent<?>> iterator() {
        ...
        final ki wrapped = this.getWrapped();
        for (final ko<?> dataComponent : wrapped) {
            final alf id = this.findId((kl<?>)dataComponent.a());
            if (id == null) {
    ...
    @Nullable
    private alf findId(final kl<?> type) {
        return lp.as.b((Object)type);
    }
    
    @Nullable
    private kl<?> findType(final alf location) {
