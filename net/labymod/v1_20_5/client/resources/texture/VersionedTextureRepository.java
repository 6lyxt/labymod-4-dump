...
public class VersionedTextureRepository extends DefaultAbstractTextureRepository implements TextureRepository
{
    ...
    public void onTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof final PathResourceLocation path) {
                final gpb textureManager = this.getTextureManager();
                final alf minecraftLocation = resourceLocation.getMinecraftLocation();
    ...
    public void register(final ResourceLocation location, final Texture texture, @Nullable final Runnable finishHandler) {
        final gol minecraftTexture = this.convertToVersionedTexture(texture);
    ...
    public void registerTexture(final ResourceLocation location, final Object texture, final Runnable finishHandler) {
        if (texture instanceof final gol minecraftTexture) {
        ...
        throw new IllegalStateException(texture.getClass().getName() + " is not a subtype of " + gol.class.getName());
    }
    
    @Override
    public void preloadTexture(final ResourceLocation location) {
        this.registerTexture(location, (gol)new got((alf)location.getMinecraftLocation()));
    }
    ...
    public Texture getTexture(final ResourceLocation location) {
        ...
        final gol texture = this.getTextureManager().b((alf)location.getMinecraftLocation());
    ...
    private void release(final ResourceLocation location, final Runnable releaseHandler) {
        this.getTextureManager().c((alf)location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull final ResourceLocation location, final gol texture) {
        this.getTextureManager().a((alf)location.getMinecraftLocation(), texture);
    }
    
    private void registerTexture(@NotNull final ResourceLocation location, final gol texture, final Runnable finishHandler) {
        this.registerTexture(location, texture);
        Runnables.runIfNotNull(finishHandler);
    }
    
    private gpb getTextureManager() {
        if (this.textureManager == null) {
            this.textureManager = ffg.Q().aa();
        }
        return this.textureManager;
    }
    
    private gol convertToVersionedTexture(final Texture texture) {
        if (texture instanceof final LabyTexture labyTexture) {
            return (gol)new VersionedLabyTexture(labyTexture);
        }
        if (texture instanceof final gol abstractTexture) {
            return abstractTexture;
        }
        throw this.createSubtypeException(texture, gol.class);
    }
}
