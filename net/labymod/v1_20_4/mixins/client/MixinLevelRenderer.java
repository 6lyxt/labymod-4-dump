...
import net.labymod.v1_20_4.client.renderer.OverlayRenderType;
...
public class MixinLevelRenderer implements WorldRenderer
{
    ...
    public void labyMod$renderCustomOutline(final ftf instance, final eqb poseStack, final eqf vertexConsumer, final blv entity, final double x, final double y, final double z, final hx blockPos, final djh blockState) {
        ...
        final emm voxelShape = blockState.a((csv)this.u, blockPos, ely.a(entity));
        ...
        if (event.getOverlayColor() != null) {
            final eqf boxVertex = this.t.c().getBuffer((ftp)OverlayRenderType.INSTANCE);
    ...
    private emj.a drawLines(final eqf vertexConsumer, final eqb.a pose, final double x, final double y, final double z, final Color color) {
        ...
        return (minX, minY, minZ, maxX, maxY, maxZ) -> {
            ...
            final float offset = auo.c(blockX * blockX + blockY * blockY + blockZ * blockZ);
            blockX /= offset;
            blockY /= offset;
            blockZ /= offset;
    ...
    private void labyMod$beforeEntityRenderPass(final eqb param0, final float param1, final long param2, final boolean param3, final eut param4, final fta param5, final ftg param6, final Matrix4f param7, final CallbackInfo ci) {
    ...
    private void labyMod$afterEntityRenderPass(final eqb param0, final float param1, final long param2, final boolean param3, final eut param4, final fta param5, final ftg param6, final Matrix4f param7, final CallbackInfo ci) {
        Laby.fireEvent(new EntityRenderPassEvent(EntityRenderPassEvent.Phase.AFTER));
    }
    
    private emj.a drawBoxes(final eqf vertexConsumer, final eqb.a pose, final double x, final double y, final double z, final Color color) {
        final float red = color.getRed() / 255.0f;
        final float green = color.getGreen() / 255.0f;
        final float blue = color.getBlue() / 255.0f;
        final float alpha = color.getAlpha() / 255.0f;
    ...
    private eqf vertex(final eqf vertexConsumer, final Matrix4f matrix, final double x, final double y, final double z) {
