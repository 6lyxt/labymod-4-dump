...
public abstract class MixinMouseHandler implements MouseHandlerAccessor
{
    ...
    private void labyMod$handleMouseScroll(final long windowHandle, final double xOffset, final double yOffset, final InsertInfo callback) {
        final evm options = evi.O().m;
        final double scrollOffset = options.P().c() ? Math.signum(yOffset) : yOffset;
        final double delta = scrollOffset * (double)options.D().c();
    ...
    private void redirectPlayerRotation(final fsj player, final double x, final double y) {
    ...
    public void grabMouseNative() {
        eow.a(this.a.aM().i(), 212995, this.a.aM().m() / 2.0, this.a.aM().n() / 2.0);
    }
    
    @Override
    public void ungrabMouseNative() {
        eow.a(this.a.aM().i(), 212993, this.a.aM().m() / 2.0, this.a.aM().n() / 2.0);
    }
    
    @Redirect(method = { "onScroll" }, at = @At(value = "FIELD", ordinal = 5, target = "Lnet/minecraft/client/MouseHandler;accumulatedScrollY:D"))
    private double labyMod$fireHotbarSlotChangeEvent(final evj instance) {
        final double delta = this.q;
        if (delta == 0.0) {
            return 0.0;
        }
        final fsj player = this.a.s;
        final int slotDelta = MathHelper.clamp((int)delta, -1, 1);
        final ClientHotbarSlotChangeEvent event = new ClientHotbarSlotChangeEvent(player.fS().l, slotDelta);
