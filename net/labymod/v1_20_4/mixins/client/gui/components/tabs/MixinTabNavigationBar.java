...
public abstract class MixinTabNavigationBar<T extends AbstractWidget<?>> extends eyz implements ConvertableMinecraftWidget<T>, TabNavigationBarAccessor
{
    ...
    private final Map<ezh, VersionedTab> versionedTabs;
    @Shadow
    @Final
    ...
    public MixinTabNavigationBar() {
        ...
        this.versionedTabs = new HashMap<ezh, VersionedTab>();
    }
    ...
    public VersionedTab versionedTabOf(final ezh tab) {
        return this.versionedTabs.computeIfAbsent(tab, VersionedTab::new);
    }
    
    @Insert(method = { "render" }, at = @At("HEAD"), cancellable = true)
    public void render(final ewu graphics, final int mouseX, final int mouseY, final float partialTicks, final InsertInfo ci) {
        final ezh tab = this.h.a();
