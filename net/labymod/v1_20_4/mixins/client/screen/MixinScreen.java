...
public abstract class MixinScreen implements FileDropHandler, ScreenAccessor
{
    ...
    public void fireScreenInitEvent(final evi mc, final int width, final int height, final InsertInfo ci) {
        Laby.fireEvent(new VersionedScreenInitEvent(this));
    }
    
    @Insert(method = { "renderTransparentBackground" }, at = @At("HEAD"), cancellable = true)
    public void renderThemeBackground(final ewu graphics, final InsertInfo ci) {
    ...
    public void renderThemeDirtBackground(final ewu graphics, final InsertInfo ci) {
        this.renderThemeBackground(((VanillaStackAccessor)graphics.c()).stack(), ci);
    }
    ...
    private void renderThemeBackground(Stack stack, final InsertInfo ci) {
        ...
        stack = ((stack != null) ? stack : ((VanillaStackAccessor)new eqb()).stack());
    ...
    private void labyMod$disableMenuBackground(final ewu graphics, final int mouseX, final int mouseY, final float partialTicks, final InsertInfo ci) {
    ...
    public void renderComponentHoverEffect(final Stack stack, final Style style, final int x, final int y) {
        if (this.labyMod$graphics == null) {
            this.labyMod$graphics = new ewu(this.f, this.f.aO().c());
        }
        ((GuiGraphicsAccessor)this.labyMod$graphics).setPose((eqb)stack.getProvider().getPoseStack());
        this.labyMod$graphics.a(this.i, (wc)style, x, y);
    }
    
    @Override
    public void setMinecraft(final evi minecraft) {
        this.f = minecraft;
    }
    ...
    public List<Object> getWrappedWidgets() {
        ...
        for (final ezb child : this.k) {
            wrappedWidgets.add(this.asVersionedWidget(child));
        }
        return wrappedWidgets;
    }
    
    private VersionedWidget asVersionedWidget(final ezb child) {
        ...
        if (child instanceof final eza container) {
            for (final ezb entry : container.l()) {
