...
package net.labymod.v1_20_4.client.font.text.vanilla;
...
import net.labymod.v1_20_4.client.font.DynamicStringRenderOutput;
...
public class VanillaTextRenderer extends AbstractTextRenderer
{
    ...
    public float width(final int codepoint, final Style style) {
        final evu.f provider = ((StringSplitterAccessor)this.font().b()).getWidthProvider();
        if (style == null || style.isEmpty()) {
            return provider.getWidth(codepoint, wc.a);
        }
        return provider.getWidth(codepoint, (wc)style);
    }
    ...
    public float width(final VisualCharSequence text) {
        return (float)this.font().a((aua)text.castTo(aua.class));
    }
    ...
    public TextBuffer render(final Stack stack) {
        ...
        if (this.isAlphaAboveThreshold(color)) {
            ...
            if (!hasMultiBufferSource) {
                multiBufferSource = evi.O().aO().c();
            }
            stringWidth = this.renderComponent(stack, (fth)multiBufferSource, component, x, y, color);
            if (!hasMultiBufferSource) {
                ((fth.a)multiBufferSource).b();
            }
        }
        else {
    ...
    public void beginBatch(final Stack stack) {
        if (stack.getMultiBufferSource() == null) {
            stack.multiBufferSource(evi.O().aO().c());
    ...
    public void endBatch(final Stack stack) {
        if (this.bufferSourceAdded) {
            ...
            if (multiBufferSource instanceof final fth.a bufferSource) {
                bufferSource.b();
            }
    ...
    private float renderComponent(final Stack stack, final fth bufferSource, final RenderableComponent component, final float x, final float y, final int color) {
    ...
    public float getWidth(final RenderableComponent component) {
        ...
        float finalWidth = 0.0f;
        for (final FormattedRenderableComponent formattedRenderableComponent : formattedRenderableComponents) {
            final aua formattedText = this.composite(formattedRenderableComponent);
    ...
    private void renderFormattedComponent(final Matrix4f modelViewMatrix, final DynamicStringRenderOutput output, final FormattedRenderableComponent component, final float x, final float y, final int color) {
        for (final RenderableComponent child : component.components()) {
            final aua sequence = MinecraftUtil.fromText(child.getText(), child.style());
    ...
    private void renderText(final DynamicStringRenderOutput output, final aua sequence, final float x, final float y, final int color, final boolean dropShadow, final Matrix4f modelViewMatrix) {
        ...
        if (dropShadow) {
            ...
            sequence.accept((aub)output);
            copiedModelViewMatrix.translate((Vector3fc)ews.e);
        }
