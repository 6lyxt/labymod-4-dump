...
public class SliderConverter extends AbstractMinecraftWidgetConverter<exc, SliderWidget>
{
    ...
    @Override
    public SliderWidget createDefault(final exc source) {
        final SliderWidget destination = new SliderWidget().withFormatter(() -> this.mapComponent(source.x()));
    ...
    @Override
    public void update(final exc source, final SliderWidget destination) {
    ...
    @Override
    public String getWidgetId(final exc source) {
        return this.componentMapper.getTranslationKeyOfComponent(source.x());
    }
    
    @Override
    public List<String> getWidgetIds(final exc source) {
