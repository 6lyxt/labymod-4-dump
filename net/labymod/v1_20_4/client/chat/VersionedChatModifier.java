...
public class VersionedChatModifier implements InternalChatModifier
{
    ...
    public void fireChatClear(final boolean clearHistory) {
        evi.O().l.d().a(clearHistory);
    }
    
    @Override
    public void deleteMessage(final ChatMessage message) {
        final VersionedChatComponent accessor = (VersionedChatComponent)evi.O().l.d();
    ...
    public void editMessage(final ChatMessage message, final Component component) {
        ...
        final evc prevMessage = accessor.getMessages().get(index);
        final vf mapped = (vf)new VersionedChatMessageComponent(message, (vf)this.componentMapper.toMinecraftComponent(component));
        accessor.getMessages().set(index, new evc(prevMessage.a(), mapped, prevMessage.c(), prevMessage.d()));
        final Predicate<evc.a> predicate = this.formattedFilter(message);
        int first = -1;
    ...
    private Predicate<evc> messageFilter(final ChatMessage message) {
    ...
    private Predicate<evc.a> formattedFilter(final ChatMessage message) {
