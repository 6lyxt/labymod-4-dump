...
import net.labymod.v1_20_4.client.gfx.buffer.VersionedBlaze3DBufferSource;
...
public class VersionedGFXBridge extends LWJGL3GFXBridge
{
    ...
    public void bindResourceLocation(final ResourceLocation location, final boolean blur, final boolean mipmap) {
        ...
        final geo textureManager = evi.O().Y();
        final gdy texture = textureManager.b((ahg)location.getMinecraftLocation());
    ...
    public void setCursorPosition(double x, double y) {
        ...
        GLFW.glfwSetCursorPos(evi.O().aM().i(), Math.max(Math.min(x, window.getRawWidth()), 0.0), Math.max(Math.min(y, window.getRawHeight()), 0.0));
    }
