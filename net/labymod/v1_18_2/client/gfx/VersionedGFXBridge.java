...
import net.labymod.v1_18_2.client.gfx.buffer.VersionedBlaze3DBufferSource;
...
public class VersionedGFXBridge extends LWJGL3GFXBridge
{
    ...
    public void bindResourceLocation(final ResourceLocation location, final boolean blur, final boolean mipmap) {
        ...
        final faz textureManager = dyr.D().M();
        final fam texture = textureManager.b((yt)location.getMinecraftLocation());
    ...
    public void setCursorPosition(double x, double y) {
        ...
        GLFW.glfwSetCursorPos(dyr.D().aC().i(), Math.max(Math.min(x, window.getRawWidth()), 0.0), Math.max(Math.min(y, window.getRawHeight()), 0.0));
    }
