...
package net.labymod.v1_18_2.client.font.text.vanilla;

import org.jetbrains.annotations.NotNull;
import java.util.ArrayList;
import java.util.Iterator;
import java.util.List;
import net.labymod.v1_18_2.client.font.DynamicStringRenderOutput;
...
public class VanillaTextRenderer extends AbstractTextRenderer
{
    ...
    public float width(final int codepoint, final Style style) {
        final dze.f provider = ((StringSplitterAccessor)this.font().b()).getWidthProvider();
        if (style == null || style.isEmpty()) {
            return provider.getWidth(codepoint, qu.a);
        }
        return provider.getWidth(codepoint, (qu)style);
    }
    ...
    public float width(final VisualCharSequence text) {
        return (float)this.font().a((aiz)text.castTo(aiz.class));
    }
    ...
    public TextBuffer render(final Stack stack) {
        ...
        if (this.isAlphaAboveThreshold(color)) {
            ...
            if (!hasMultiBufferSource) {
                multiBufferSource = dyr.D().aD().b();
            }
            stringWidth = this.renderComponent(stack, (eqs)multiBufferSource, component, x, y, color);
            if (!hasMultiBufferSource) {
                ((eqs.a)multiBufferSource).b();
            }
        }
        else {
    ...
    public void beginBatch(final Stack stack) {
        if (stack.getMultiBufferSource() == null) {
            stack.multiBufferSource(dyr.D().aD().b());
    ...
    public void endBatch(final Stack stack) {
        if (this.bufferSourceAdded) {
            ...
            if (multiBufferSource instanceof final eqs.a bufferSource) {
                bufferSource.b();
            }
    ...
    private float renderComponent(final Stack stack, final eqs bufferSource, final RenderableComponent component, final float x, final float y, final int color) {
    ...
    public float getWidth(final RenderableComponent component) {
        ...
        float finalWidth = 0.0f;
        for (final FormattedRenderableComponent formattedRenderableComponent : formattedRenderableComponents) {
            final aiz formattedText = this.composite(formattedRenderableComponent);
    ...
    @NotNull
    private aiz composite(final FormattedRenderableComponent component) {
        final List<RenderableComponent> components = component.components();
        final List<aiz> sequences = new ArrayList<aiz>(components.size());
        for (final RenderableComponent child : components) {
            sequences.add(MinecraftUtil.fromText(child.getText(), child.style()));
        }
        return aiz.composite((List)sequences);
    }
    
    private void renderFormattedComponent(final d modelViewMatrix, final DynamicStringRenderOutput output, final FormattedRenderableComponent component, final float x, final float y, final int color) {
        for (final RenderableComponent child : component.components()) {
            final aiz sequence = MinecraftUtil.fromText(child.getText(), child.style());
    ...
    private void renderText(final DynamicStringRenderOutput output, final aiz sequence, final float x, final float y, final int color, final boolean dropShadow, final d modelViewMatrix) {
        final d copiedModelViewMatrix = new d(modelViewMatrix);
        if (dropShadow) {
            output.updateModelViewMatrix(modelViewMatrix);
            output.updateColor(color, true);
            sequence.accept((aja)output);
            copiedModelViewMatrix.a(dzp.e);
        }
