...
public class DynamicFont extends dzp implements FontHeightAccessor
{
    ...
    private final dzp delegate;
    private final dze customSplitter;
    
    public DynamicFont(final dzp delegate) {
        super(delegate.f);
        this.delegate = delegate;
        this.customSplitter = new dze((codepoint, style) -> DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().width(codepoint, (Style)style));
    }
    
    @NotNull
    public ebr a(@NotNull final yt location) {
    ...
    public int b(final String text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final d modelViewMatrix, @NotNull final eqs bufferSource, final boolean seeThrough, final int backgroundColor, final int packedLightCoords, final boolean bidirectionalShaping) {
    ...
    public int b(@NotNull final aiz text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final d modelViewMatrix, @NotNull final eqs bufferSource, final boolean seeThrough, final int backgroundColor, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
    ...
    public void a(@NotNull final aiz text, final float x, final float y, final int color, final int outlineColor, @NotNull final d modelViewMatrix, @NotNull final eqs bufferSource, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
        ...
        builder.pos(x, y).text(labyComponent).drawMode(MinecraftUtil.fromMinecraft(dzp.a.c)).color(color).shadow(false).render(stack);
        stack.pop();
    }
    ...
    public int a(@NotNull final qn text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
    ...
    public int a(@NotNull final aiz text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
        final StringOutput stringOutput = new StringOutput();
        text.accept((aja)stringOutput);
        return this.a((qn)DynamicFont.COMPONENT_MAPPER.toMinecraftComponent(stringOutput.getLabyComponent()));
    }
    ...
    public String a(@NotNull final String text, final int maxWidth, final boolean tail) {
        ...
        if (tail) {
            return this.customSplitter.c(text, maxWidth, qu.a);
        }
        return this.customSplitter.b(text, maxWidth, qu.a);
    }
    ...
    @NotNull
    public qn a(@NotNull final qn text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text, maxWidth);
        }
        return this.customSplitter.a(text, maxWidth, qu.a);
    }
    ...
    public int b(@NotNull final String text, final int maxWidth) {
        ...
        final int lines = this.customSplitter.g(text, maxWidth, qu.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    @NotNull
    public List<aiz> b(@NotNull final qn text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.b(text, maxWidth);
        }
        return of.a().a(this.customSplitter.b(text, maxWidth, qu.a));
    }
    ...
    static class StringOutput implements aja
    {
        ...
        public boolean accept(final int position, @NotNull final qu style, final int codepoint) {
