...
public class VersionedResourcePackScanner extends AbstractResourcePackScanner
{
    ...
    private boolean isExternalPack(final afa pack) {
        return pack instanceof aey || pack instanceof aez;
    }
    
    private IncompatibleResourcePack scanPack(final afa pack) {
    ...
    private void scanPack(final afa pack, final String baseDirectory, final Consumer<yt> output) {
        final Collection<yt> locations = pack.a(afb.a, "minecraft", baseDirectory, Integer.MAX_VALUE, s -> true);
        for (final yt location : locations) {
            output.accept(location);
        }
    }
    
    private Collection<String> collectIgnoredFiles(final afa pack) {
        final Set<String> ignoredFiles = new HashSet<String>();
        for (final String namespace : pack.a(afb.a)) {
            InputStream resource;
            try {
                resource = pack.a(afb.a, new yt(namespace, "labymod/ignored_files.json"));
            }
