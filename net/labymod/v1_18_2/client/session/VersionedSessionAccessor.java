...
public class VersionedSessionAccessor extends DefaultAbstractSessionAccessor<dzh>
{
    ...
    @Override
    public dzh toGameSession(final Session session) {
        return (dzh)this.userFactory.createUser(session);
    }
    
    @Override
    public Session fromGameSession(final dzh gameSession) {
        return (gameSession == null) ? null : this.createSession(gameSession.c(), gameSession.g().getId(), gameSession.d());
    }
