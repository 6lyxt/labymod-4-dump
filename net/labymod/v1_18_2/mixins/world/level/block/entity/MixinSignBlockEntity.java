...
public abstract class MixinSignBlockEntity implements SignBlockEntity
{
    ...
    private void labyMod$fireTileEntityPostLoadEvent(final ok tag, final InsertInfo ci) {
        Laby.fireEvent(new SignBlockEntityPostLoadEvent(this, (NBTTagCompound)tag));
    }
    
    @Override
    public float getRotationYaw() {
        final cmr entity = (cmr)this;
        final cov state = entity.q();
        final cdq block = state.b();
        if (block instanceof clp) {
            final go facing = (go)state.c((cpy)clp.d);
        ...
        if (block instanceof cko) {
            final Integer rotation = (Integer)state.c((cpy)cko.d);
