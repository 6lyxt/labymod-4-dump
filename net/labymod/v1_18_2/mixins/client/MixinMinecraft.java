...
public abstract class MixinMinecraft
{
    ...
    private Proxy labyMod$callGameInitializeResource(final eii.d userData) {
    ...
    private String labyMod$censorSessionId(final dzh instance) {
        return "********************************";
    }
    ...
    private File labyMod$onPreGameStarted(final eii.a instance) {
    ...
    private dsr labyMod$adjustWindowSize(final erh instance, dse displayData, final String $$1, final String $$2) {
        ...
        if (sizePair != null) {
            displayData = new dse((int)sizePair.getFirst(), (int)sizePair.getSecond(), displayData.c, displayData.d, displayData.e);
        }
    ...
    private void labyMod$maximizeDisplay(final eii $$0, final CallbackInfo ci) {
    ...
    private void labyMod$firePostGameStartedInitializeEvent(final eii param0, final CallbackInfo ci) {
    ...
    private void labyMod$cancelRemovedScreen(final edw screen) {
    }
    ...
    private ScreenWrapper labyMod$createScreenWrapper(final edw screen) {
    ...
    private edj labyMod$createLoadingOverlay(final dyr minecraft, final afx reloadInstance, final Consumer onFinish, final boolean fadeIn) {
        ...
        return (type == FadeOutAnimationType.FADING && !DynamicBackgroundController.isEnabled()) ? new edj(minecraft, reloadInstance, onFinish, fadeIn) : new CustomLoadingOverlay(minecraft, reloadInstance, onFinish, fadeIn);
    }
    
    @Insert(method = { "clearLevel(Lnet/minecraft/client/gui/screens/Screen;)V" }, at = @At("HEAD"))
    private void labyMod$firePreNetworkDisconnectEvent(final edw screen, final InsertInfo ci) {
    ...
    private void labyMod$firePostNetworkDisconnectEvent(final edw screen, final InsertInfo ci) {
    ...
    private void labyMod$firePreWorldLoadEvent(final ems level, final InsertInfo ci) {
        if (level != null) {
    ...
    private void labyMod$firePostWorldLoadEvent(final ems level, final InsertInfo ci) {
        if (level != null) {
    ...
    private void labyMod$fireGameRenderEvent(final eql gameRenderer, final float partialTicks, final long nanoTime, final boolean running) {
        final dtm poseStack = new dtm();
    ...
    @Overwrite
    public static void a(final q report) {
        final File dir = new File(D().o, "crash-reports");
        ...
        yv.a(report.e());
        if (report.f() != null) {
            yv.a("#@!@# Game crashed! Crash report saved to: #@!@# " + String.valueOf(report.f()));
            System.exit(-1);
        }
        else if (report.a(crashFile)) {
            yv.a("#@!@# Game crashed! Crash report saved to: #@!@# " + crashFile.getAbsolutePath());
            System.exit(-1);
        }
        else {
            yv.a("#@?@# Game crashed! Crash report could not be saved. #@?@#");
            System.exit(-2);
        }
    }
}
