...
public class MixinLevelRenderer implements WorldRenderer
{
    ...
    public void labyMod$renderCustomOutline(final eqq instance, final dtm poseStack, final dtq vertexConsumer, final axk entity, final double x, final double y, final double z, final gj blockPos, final cov blockState) {
        ...
        final dqh voxelShape = blockState.a((cab)this.x, blockPos, dpt.a(entity));
        ...
        if (event.getOverlayColor() != null) {
            final dtq boxVertex = this.w.b().getBuffer((era)OverlayRenderType.INSTANCE);
    ...
    private dqe.a drawLines(final dtq vertexConsumer, final dtm.a pose, final double x, final double y, final double z, final Color color) {
        ...
        return (minX, minY, minZ, maxX, maxY, maxZ) -> {
            ...
            final float offset = ajl.c(blockX * blockX + blockY * blockY + blockZ * blockZ);
            blockX /= offset;
            blockY /= offset;
            blockZ /= offset;
    ...
    private void labyMod$beforeEntityRenderPass(final dtm param0, final float param1, final long param2, final boolean param3, final dyb param4, final eql param5, final eqr param6, final d param7, final CallbackInfo ci) {
    ...
    private void labyMod$afterEntityRenderPass(final dtm param0, final float param1, final long param2, final boolean param3, final dyb param4, final eql param5, final eqr param6, final d param7, final CallbackInfo ci) {
        Laby.fireEvent(new EntityRenderPassEvent(EntityRenderPassEvent.Phase.AFTER));
    }
    
    private dqe.a drawBoxes(final dtq vertexConsumer, final dtm.a pose, final double x, final double y, final double z, final Color color) {
        final float red = color.getRed() / 255.0f;
        final float green = color.getGreen() / 255.0f;
        final float blue = color.getBlue() / 255.0f;
        final float alpha = color.getAlpha() / 255.0f;
    ...
    private dtq vertex(final dtq vertexConsumer, final d matrix, final double x, final double y, final double z) {
