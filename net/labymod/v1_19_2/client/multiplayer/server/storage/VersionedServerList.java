...
package net.labymod.v1_19_2.client.multiplayer.server.storage;
...
public class VersionedServerList extends DefaultServerList
{
    ...
    public void load() {
        try {
            ...
            final pj root = pt.b(new File(this.gameDirectory, "servers.dat"));
            if (root == null) {
                return;
            }
            final pp list = root.c("servers", 10);
    ...
    private pj serverDataToTag(final StorageServerData data) {
        final pj tag = new pj();
        ...
        if (data.hasMetadata()) {
            ...
            tag.a("labyModMetadata", (qc)metaTag);
        }
        return tag;
    }
    
    private StorageServerData tagToServerData(final pj tag) {
        ...
        if (tag.b("labyModMetadata", 10)) {
            final pj metaTag = tag.p("labyModMetadata");
            for (final String key : metaTag.d()) {
                final qc metaValue = metaTag.c(key);
                String value;
                if (metaValue instanceof qa) {
                    value = metaValue.e_();
                }
                else {
                    if (!(metaValue instanceof pg)) {
                        continue;
                    }
                    final pg byteArrayTag = (pg)metaValue;
