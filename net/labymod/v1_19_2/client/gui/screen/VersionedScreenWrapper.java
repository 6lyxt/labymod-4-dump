...
{
    private final elm screen;
    private Metadata metadata;
    private boolean initialized;
    
    public VersionedScreenWrapper(final elm screen) {
        if (screen == null) {
    ...
    public void initialize(final Parent parent) {
        if (parent != null) {
            this.screen.b(efu.I(), (int)parent.bounds().getWidth(BoundsType.INNER), (int)parent.bounds().getHeight(BoundsType.INNER));
        }
        else {
            final Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(efu.I(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void render(final Stack stack, final MutableMouse mouse, final float partialTicks) {
        this.screen.a((eaq)stack.getProvider().getPoseStack(), mouse.getX(), mouse.getY(), partialTicks);
    }
    ...
    public void resize(final int width, final int height) {
        this.screen.b(efu.I(), width, height);
        this.initialized = true;
    }
    ...
    public void render(final ScreenContext context) {
        ...
        final eaq poseStack = (eaq)context.stack().getProvider().getPoseStack();
