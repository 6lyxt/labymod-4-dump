...
public class ButtonConverter extends AbstractMinecraftWidgetConverter<ehi, ButtonWidget>
{
    ...
    @Override
    public ButtonWidget createDefault(final ehi source) {
    ...
    @Override
    public void update(final ehi source, final ButtonWidget destination) {
        destination.setVisible(source.q);
        destination.setEnabled(source.p);
        destination.setFocused(source.n());
        this.copyBounds(source, destination);
    }
    
    @Override
    public String getWidgetId(final ehi source) {
        return this.componentMapper.getTranslationKeyOfComponent(source.k());
    }
    
    @Override
    public List<String> getWidgetIds(final ehi source) {
    ...
    class ImageButtonMapper implements Mapper<ehi, ButtonWidget>
    {
        @Override
        public ButtonWidget map(final ehi source) {
            if (source instanceof final ehy imageButton) {
    ...
    class LockIconButtonMapper implements Mapper<ehi, ButtonWidget>
    {
        ...
        @Override
        public ButtonWidget map(final ehi source) {
            if (source instanceof final eia lockIconButton) {
                ...
                final boolean enabled = lockIconButton.aj_();
