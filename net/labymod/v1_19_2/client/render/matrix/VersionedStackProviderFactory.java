...
public class VersionedStackProviderFactory implements StackProviderFactory
{
    ...
    @Override
    public Stack create() {
        return Stack.create(new VersionedStackProvider(new eaq()));
    }
    
    @Override
    public Stack create(final Object poseStack) {
        Objects.requireNonNull(poseStack, "poseStack must not be null");
        if (poseStack instanceof final eaq stack) {
            return Stack.create(new VersionedStackProvider(stack));
        }
        throw new IllegalStateException(poseStack.getClass().getName() + " is not an instance of " + String.valueOf(eaq.class));
    }
