...
public class VersionedPlayerHeartRenderer implements PlayerHeartRenderer
{
    ...
    public VersionedPlayerHeartRenderer(final LabyAPI labyAPI, final EventBus eventBus, final ResourceRenderer resourceRenderer) {
        ...
        this.guiIconsLocation = (ResourceLocation)ehc.i;
    }
    ...
    public void renderHearts(final Stack stack, final float rawX, final float rawY, final int heartSize, final NetworkPlayerInfo info) {
        final euz player = ((VersionedNetworkPlayerInfo)info).getWrapped();
        final int health = info.getHealth();
        RenderSystem.setShaderTexture(0, ehc.i);
        final long millis = ad.b();
        if (this.visibilityId == player.s()) {
            if (health < player.o()) {
                player.a(millis);
                player.b((long)(this.tickCount + 20));
            }
            else if (health > player.o()) {
                player.a(millis);
                player.b((long)(this.tickCount + 10));
            }
        }
        if (millis - player.q() > 1000L) {
            player.b(health);
            player.c(health);
            player.a(millis);
        }
        ...
        player.b(health);
        final int availableHearts = MathHelper.ceil(Math.max(health, player.p()) / 2.0f);
        final int heartAmount = Math.max(MathHelper.ceil((float)(health / 2)), Math.max(MathHelper.ceil((float)(player.p() / 2)), 10));
        final boolean blinkLight = player.r() > this.tickCount && (player.r() - this.tickCount) / 3L % 2L == 1L;
