...
package net.labymod.v1_19_2.client.session;
...
public class VersionedSessionAccessor extends DefaultAbstractSessionAccessor<egk>
{
    ...
    @Override
    public egk toGameSession(final Session session) {
        return (egk)this.userFactory.createUser(session);
    }
    
    @Override
    public Session fromGameSession(final egk gameSession) {
        return (gameSession == null) ? null : this.createSession(gameSession.c(), gameSession.h().getId(), gameSession.d());
    }
    
    @Override
    public egk getGameSession() {
        return efu.I().O();
    }
}
