...
public class DynamicFont extends eha implements FontHeightAccessor
{
    ...
    private final eha delegate;
    private final egh customSplitter;
    
    public DynamicFont(final eha delegate) {
        super(delegate.f, delegate.g);
        this.delegate = delegate;
        this.customSplitter = new egh((codepoint, style) -> DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().width(codepoint, (Style)style));
    }
    
    @NotNull
    public ejf a(@NotNull final abb location) {
    ...
    public int b(final String text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final d modelViewMatrix, @NotNull final ezs bufferSource, final boolean seeThrough, final int backgroundColor, final int packedLightCoords, final boolean bidirectionalShaping) {
    ...
    public int b(@NotNull final alu text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final d modelViewMatrix, @NotNull final ezs bufferSource, final boolean seeThrough, final int backgroundColor, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
    ...
    public void a(@NotNull final alu text, final float x, final float y, final int color, final int outlineColor, @NotNull final d modelViewMatrix, @NotNull final ezs bufferSource, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
        ...
        builder.pos(x, y).text(labyComponent).drawMode(MinecraftUtil.fromMinecraft(eha.a.c)).color(color).shadow(false).render(stack);
        stack.pop();
    }
    ...
    public int a(@NotNull final ru text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
    ...
    public int a(@NotNull final alu text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
        final StringOutput stringOutput = new StringOutput();
        text.accept((alv)stringOutput);
        return this.a((ru)DynamicFont.COMPONENT_MAPPER.toMinecraftComponent(stringOutput.getLabyComponent()));
    }
    ...
    public String a(@NotNull final String text, final int maxWidth, final boolean tail) {
        ...
        if (tail) {
            return this.customSplitter.c(text, maxWidth, sj.a);
        }
        return this.customSplitter.b(text, maxWidth, sj.a);
    }
    ...
    @NotNull
    public ru a(@NotNull final ru text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text, maxWidth);
        }
        return this.customSplitter.a(text, maxWidth, sj.a);
    }
    ...
    public int b(@NotNull final String text, final int maxWidth) {
        ...
        final int lines = this.customSplitter.g(text, maxWidth, sj.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    public int b(@NotNull final ru text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.b(text, maxWidth);
        }
        final int lines = this.customSplitter.b(text, maxWidth, sj.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    @NotNull
    public List<alu> c(@NotNull final ru text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.c(text, maxWidth);
        }
        return pe.a().a(this.customSplitter.b(text, maxWidth, sj.a));
    }
    ...
    static class StringOutput implements alv
    {
        ...
        public boolean accept(final int position, @NotNull final sj style, final int codepoint) {
