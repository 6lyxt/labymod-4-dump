...
package net.labymod.v1_19_2.mixins.server.network;
...
public class MixinServerGamePacketListenerImpl
{
    ...
    private void labyMod$fireDisconnectEventPre(final rq reason, final InsertInfo ci) {
        final GameProfile profile = (GameProfile)this.c.fy();
        Laby.fireEvent(new IntegratedServerPlayerDisconnectEvent(Phase.PRE, profile));
    }
    
    @Insert(method = { "onDisconnect" }, at = @At("TAIL"))
    private void labyMod$fireDisconnectEventPost(final rq reason, final InsertInfo ci) {
        final GameProfile profile = (GameProfile)this.c.fy();
