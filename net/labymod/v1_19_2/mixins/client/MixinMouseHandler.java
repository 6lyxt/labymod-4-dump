...
public abstract class MixinMouseHandler implements MouseHandlerAccessor
{
    ...
    private void labyMod$handleMouseScroll(final long windowHandle, final double xOffset, final double yOffset, final InsertInfo callback) {
        final efy options = efu.I().m;
        final double scrollOffset = options.J().c() ? Math.signum(yOffset) : yOffset;
        final double delta = scrollOffset * (double)options.y().c();
    ...
    private void redirectPlayerRotation(final eyw player, final double x, final double y) {
    ...
    public void grabMouseNative() {
        dzk.a(this.a.aG().i(), 212995, this.a.aG().m() / 2.0, this.a.aG().n() / 2.0);
    }
    
    @Override
    public void ungrabMouseNative() {
        dzk.a(this.a.aG().i(), 212993, this.a.aG().m() / 2.0, this.a.aG().n() / 2.0);
    }
    
    @Redirect(method = { "onScroll" }, at = @At(value = "FIELD", ordinal = 5, target = "Lnet/minecraft/client/MouseHandler;accumulatedScroll:D"))
    private double labyMod$fireHotbarSlotChangeEvent(final efv instance) {
        final double delta = this.p;
        if (delta == 0.0) {
            return 0.0;
        }
        final eyw player = this.a.t;
        final int slotDelta = MathHelper.clamp((int)delta, -1, 1);
        final ClientHotbarSlotChangeEvent event = new ClientHotbarSlotChangeEvent(player.fA().k, slotDelta);
        ...
        if (event.toSlot() != originalToSlot && !player.B_()) {
            player.fA().k = MathHelper.clamp(event.toSlot(), 0, 8);
            return 0.0;
        }
        return delta;
    }
