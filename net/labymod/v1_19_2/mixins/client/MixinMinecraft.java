...
import net.labymod.v1_19_2.client.overlay.CustomLoadingOverlay;
...
public abstract class MixinMinecraft
{
    ...
    private Proxy labyMod$callGameInitializeResource(final eqh.d userData) {
    ...
    private String labyMod$censorSessionId(final egk instance) {
        return "********************************";
    }
    ...
    private File labyMod$onPreGameStarted(final eqh.a instance) {
    ...
    private dzu labyMod$adjustWindowSize(final fah instance, dzh displayData, final String $$1, final String $$2) {
        ...
        if (sizePair != null) {
            displayData = new dzh((int)sizePair.getFirst(), (int)sizePair.getSecond(), displayData.c, displayData.d, displayData.e);
        }
    ...
    private void labyMod$maximizeDisplay(final eqh $$0, final CallbackInfo ci) {
    ...
    private void labyMod$firePostGameStartedInitializeEvent(final eqh param0, final CallbackInfo ci) {
    ...
    private String labyMod$modifiedTitle(final StringBuilder stringBuilder) {
        ...
        final euw listener = this.D();
        if (listener != null && listener.a().h()) {
            extraBuilder.append(" - ");
            if (this.aM != null && !this.aM.o()) {
                extraBuilder.append(fkz.a("title.singleplayer", new Object[0]));
            }
            else if (this.ak()) {
                extraBuilder.append(fkz.a("title.multiplayer.realms", new Object[0]));
            }
            else if (this.aM == null && (this.aN == null || !this.aN.d())) {
                extraBuilder.append(fkz.a("title.multiplayer.other", new Object[0]));
            }
            else {
                extraBuilder.append(fkz.a("title.multiplayer.lan", new Object[0]));
            }
        }
        return LabyMod.getInstance().createTitle("1.19.2", extraBuilder.toString());
    }
    
    @Redirect(method = { "setScreen" }, at = @At(value = "INVOKE", target = "Lnet/minecraft/client/gui/screens/Screen;removed()V"))
    private void labyMod$cancelRemovedScreen(final elm screen) {
    }
    ...
    private elm labyMod$fireScreenOpenEvent(elm newScreen) {
        ...
        try {
            ...
            newScreen = ((instance == null) ? null : ((elm)instance.wrap().getVersionedScreen()));
        }
    ...
    private ScreenWrapper labyMod$createScreenWrapper(final elm screen) {
    ...
    private ekz labyMod$createLoadingOverlay(final efu minecraft, final aii reloadInstance, final Consumer onFinish, final boolean fadeIn) {
        ...
        return (type == FadeOutAnimationType.FADING && !DynamicBackgroundController.isEnabled()) ? new ekz(minecraft, reloadInstance, onFinish, fadeIn) : new CustomLoadingOverlay(minecraft, reloadInstance, onFinish, fadeIn);
    }
    
    @Insert(method = { "clearLevel(Lnet/minecraft/client/gui/screens/Screen;)V" }, at = @At("HEAD"))
    private void labyMod$firePreNetworkDisconnectEvent(final elm screen, final InsertInfo ci) {
    ...
    private void labyMod$firePostNetworkDisconnectEvent(final elm screen, final InsertInfo ci) {
    ...
    private void labyMod$firePreWorldLoadEvent(final euv level, final InsertInfo ci) {
        if (level != null) {
    ...
    private void labyMod$firePostWorldLoadEvent(final euv level, final InsertInfo ci) {
        if (level != null) {
    ...
    private void labyMod$fireGameRenderEvent(final ezl gameRenderer, final float partialTicks, final long nanoTime, final boolean running) {
        final eaq poseStack = new eaq();
    ...
    @Overwrite
    public static void c(final q report) {
        final File dir = new File(I().p, "crash-reports");
        final File crashFile = new File(dir, "crash-" + ad.e() + "-client.txt");
        Laby.fireEvent(new GameShutdownEvent(report.b(), crashFile));
        abd.a(report.e());
        if (report.f() != null) {
            abd.a("#@!@# Game crashed! Crash report saved to: #@!@# " + String.valueOf(report.f()));
            System.exit(-1);
        }
        else if (report.a(crashFile)) {
            abd.a("#@!@# Game crashed! Crash report saved to: #@!@# " + crashFile.getAbsolutePath());
            System.exit(-1);
        }
        else {
            abd.a("#@?@# Game crashed! Crash report could not be saved. #@?@#");
            System.exit(-2);
        }
    }
}
