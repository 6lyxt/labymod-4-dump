...
public abstract class MixinAbstractSelectionList
{
    ...
    private void labyMod$getStack(final eaq stack, final int mouseX, final int mouseY, final float partialTicks, final InsertInfo ci) {
    ...
    private eau labyMod$addStack(final eal instance, final double x, final double y, final double z) {
    ...
    public void labyMod$renderBackground(final eaq stack, final int param1, final int param2, final float param3, final InsertInfo ci) {
    ...
    public void labyMod$renderTopAndBottom(final eaq stack, final int param1, final int param2, final float param3, final InsertInfo ci) {
        final Theme theme = Laby.labyAPI().themeService().currentTheme();
        if (!theme.metadata().getBoolean("hide_default_background")) {
    ...
    public void labyMod$renderList(final ehl instance, final eaq stack, final int mouseX, final int mouseY, final float partialTicks, final Operation<Void> original) {
        final Scissor scissor = Laby.labyAPI().gfxRenderPipeline().scissor();
        try {
            scissor.push(((VanillaStackAccessor)stack).stack(), (float)this.f, (float)(this.j - this.f));
