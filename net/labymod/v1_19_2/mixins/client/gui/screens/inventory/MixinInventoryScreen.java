...
@Mixin({ eng.class })
public abstract class MixinInventoryScreen extends emz<bxl>
{
    private InventorySlotRegistry labyMod$inventorySlotRegistry;
    private boolean labyMod$oldSurvivalLayoutTexture;
    
    public MixinInventoryScreen(final bxl param0, final bub param1, final rq param2) {
        super((bwm)param0, param1, param2);
    }
    ...
    private void labyMod$init(final InsertInfo info) {
        ...
        for (final byd slot : ((bxl)this.k()).i) {
    ...
    private void labyMod$setOldSurvivalLayoutTexture(final int slot, final abb location) {
        if (this.labyMod$oldSurvivalLayoutTexture) {
            RenderSystem.setShaderTexture(slot, (abb)Textures.SURVIVAL_INVENTORY_BACKGROUND.getMinecraftLocation());
            return;
        }
    ...
    @NotNull
    private byd getSlotByIndex(@NotNull final InventoryLayoutOldAnimation inventoryLayout, @NotNull final byd slot) {
