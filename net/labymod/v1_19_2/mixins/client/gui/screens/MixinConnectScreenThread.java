...
public class MixinConnectScreenThread
{
    ...
    private Optional<evy> labyMod$resolveAddress(final ewb resolver, final evz address) {
        final InetSocketAddress resolved = ConnectAddressResolver.resolve(address.a(), address.b(), (AddressResolveCallback)this.d);
        return (resolved == null) ? Optional.empty() : Optional.of(evy.a(resolved));
    }
    ...
    public rq labyMod$fireServerKickEvent(final rq reason) {
        ...
        return (rq)componentMapper.toMinecraftComponent(Laby.fireEvent(new ServerKickEvent(ConnectableServerData.from(serverData), componentMapper.fromMinecraftComponent(reason), ServerKickEvent.Context.PRE_LOGIN)).reason());
    }
}
