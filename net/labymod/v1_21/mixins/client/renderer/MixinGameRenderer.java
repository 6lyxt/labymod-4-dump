...
public abstract class MixinGameRenderer
{
    ...
    private Matrix4f labyMod$setViewMatrix(Matrix4f self, final Quaternionfc quaternion, final Operation<Matrix4f> original, final fgf deltaTracker) {
    ...
    private void labyMod$firePreRenderHandEvent(final ffy camera, final float partialTicks, final Matrix4f viewMatrix, final CallbackInfo ci) {
    ...
    private void labyMod$firePostRenderHandEvent(final ffy camera, final float partialTicks, final Matrix4f viewMatrix, final CallbackInfo ci) {
        final fbi poseStack = MinecraftUtil.levelRenderContext().getPoseStack();
    ...
    public void labyMod$fireFieldOfViewTickEvent(final InsertInfo ci) {
        float modifier = 1.0f;
        final bsr an = this.k.an();
        if (an instanceof final gdy cameraEntity) {
    ...
    private boolean labyMod$noViewBobbing(final ges instance, final fbi stack, final float partialTicks) {
