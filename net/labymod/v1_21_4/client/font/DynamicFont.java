...
public class DynamicFont extends fod implements FontHeightAccessor
{
    ...
    private final fod delegate;
    private final flu customSplitter;
    
    public DynamicFont(final fod delegate) {
        super(delegate.g, delegate.h);
        this.delegate = delegate;
        this.customSplitter = new flu((codepoint, style) -> DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().width(codepoint, (Style)style));
    }
    
    @NotNull
    public frm a(@NotNull final akv location) {
    ...
    public int a(final String text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final Matrix4f modelViewMatrix, @NotNull final glz bufferSource, @NotNull final fod.a displayMode, final int backgroundColor, final int packedLightCoords, final boolean bidirectionalShaping) {
    ...
    public int a(@NotNull final ayl text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final Matrix4f modelViewMatrix, @NotNull final glz bufferSource, @NotNull final fod.a displayMode, final int backgroundColor, final int packedLightCoords, final boolean inverseDepth) {
        if (this.isForceVanillaFont()) {
    ...
    public void a(@NotNull final ayl text, final float x, final float y, final int color, final int outlineColor, @NotNull final Matrix4f modelViewMatrix, @NotNull final glz bufferSource, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
        ...
        builder.pos(x, y).text(labyComponent).drawMode(MinecraftUtil.fromMinecraft(fod.a.c)).color(color).shadow(false).render(stack);
        stack.pop();
    }
    ...
    public int a(@NotNull final wu text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
    ...
    public int a(@NotNull final ayl text) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text);
        }
        final StringOutput stringOutput = new StringOutput();
        text.accept((aym)stringOutput);
        return this.a((wu)DynamicFont.COMPONENT_MAPPER.toMinecraftComponent(stringOutput.getLabyComponent()));
    }
    ...
    public String a(@NotNull final String text, final int maxWidth, final boolean tail) {
        ...
        if (tail) {
            return this.customSplitter.c(text, maxWidth, xm.a);
        }
        return this.customSplitter.b(text, maxWidth, xm.a);
    }
    ...
    @NotNull
    public wu a(@NotNull final wu text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.a(text, maxWidth);
        }
        return this.customSplitter.a(text, maxWidth, xm.a);
    }
    ...
    public int b(@NotNull final String text, final int maxWidth) {
        ...
        final int lines = this.customSplitter.g(text, maxWidth, xm.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    public int b(@NotNull final wu text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.b(text, maxWidth);
        }
        final int lines = this.customSplitter.b(text, maxWidth, xm.a).size();
        return MathHelper.ceil(lines * DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().height());
    }
    
    @NotNull
    public List<ayl> c(@NotNull final wu text, final int maxWidth) {
        if (this.isForceVanillaFont()) {
            return this.delegate.c(text, maxWidth);
        }
        return tl.a().a(this.customSplitter.b(text, maxWidth, xm.a));
    }
    ...
    @Deprecated
    private void endBatch(final glz bufferSource) {
        if (bufferSource instanceof final glz.a source) {
            source.a();
        }
    }
    ...
    static class StringOutput implements aym
    {
        ...
        public boolean accept(final int position, @NotNull final xm style, final int codepoint) {
