...
public class VersionedUserFactory implements UserFactory
{
    ...
    @Override
    public flw createUser(final Session session) {
        final UUID newUuidObject = UUID.fromString(session.getUniqueId().toString());
        return new flw(session.getUsername(), newUuidObject, session.getAccessToken(), (Optional)Optional.empty(), (Optional)Optional.empty(), session.isPremium() ? flw.a.c : flw.a.a);
    }
}
