...
public class VersionedChatModifier implements InternalChatModifier
{
    ...
    public void fireChatClear(final boolean clearHistory) {
        flk.Q().m.d().a(clearHistory);
    }
    
    @Override
    public void deleteMessage(final ChatMessage message) {
        final VersionedChatComponent accessor = (VersionedChatComponent)flk.Q().m.d();
    ...
    public void editMessage(final ChatMessage message, final Component component) {
        ...
        final fld prevMessage = accessor.getMessages().get(index);
        final wp mapped = (wp)new VersionedChatMessageComponent(message, (wp)this.componentMapper.toMinecraftComponent(component));
        accessor.getMessages().set(index, new fld(prevMessage.b(), mapped, prevMessage.d(), prevMessage.e()));
        final Predicate<fld.a> predicate = this.formattedFilter(message);
        int first = -1;
    ...
    private Predicate<fld> messageFilter(final ChatMessage message) {
    ...
    private Predicate<fld.a> formattedFilter(final ChatMessage message) {
