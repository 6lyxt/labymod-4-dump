...
public class LabyBiomeBlender implements dfl
{
    ...
    public LabyBiomeBlender setLevel(final dgj level) {
        this.level = level;
        return this;
    }
    ...
    public float a(@NotNull final jn direction, final boolean shade) {
    ...
    @Nullable
    public dua c_(@NotNull final ji blockPos) {
        return this.level.c_(blockPos);
    }
    
    @NotNull
    public dwy a_(@NotNull final ji blockPos) {
        return this.level.a_(blockPos);
    }
    
    @NotNull
    public eta b_(@NotNull final ji blockPos) {
    ...
    public int a(@NotNull final ji blockPos, @NotNull final dft resolver) {
        final int x = blockPos.u();
        final int y = blockPos.v();
        final int z = blockPos.w();
        final int[] rgb = new int[3];
        int total = 0;
        final ji.a currentPos = new ji.a();
    ...
    private int processZAxis(final int x, final int y, final int z, final ji.a currentPos, final dft resolver, int total, final int[] rgb, final int xIndex) {
    ...
    private int blendColors(final ji.a currentPos, final dft resolver, int total, final int[] rgb) {
        final jr<dhl> holder = (jr<dhl>)this.level.t((ji)currentPos);
        final dhl biome = (holder == null) ? null : ((dhl)holder.a());
        if (biome != null) {
