...
public class VersionedParticleController implements ParticleController
{
    ...
    public void crackBlock(final FloatVector3 blockPosition, final Direction direction) {
        final ji position = new ji(MathHelper.ceil(blockPosition.getX()), MathHelper.ceil(blockPosition.getY()), MathHelper.ceil(blockPosition.getZ()));
        flk.Q().g.a(position, this.getMinecraftDirection(direction));
    }
    
    public jn getMinecraftDirection(final Direction direction) {
        switch (direction) {
            case DOWN: {
                return jn.a;
            }
