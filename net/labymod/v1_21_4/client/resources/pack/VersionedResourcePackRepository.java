...
public class VersionedResourcePackRepository extends AbstractResourcePackRepository<atx>
{
    ...
    private void checkServerPack(final atx pack) {
        try (final atc resources = pack.f()) {
            final atc unwrappedPackResources = PackUtil.unwrap(resources);
            if ((unwrappedPackResources instanceof asz || unwrappedPackResources instanceof asv) && unwrappedPackResources.b().startsWith("server")) {
    ...
    protected void onReloadPackDetails() {
        ...
        final Collection<atx> packs = flk.Q().ad().d();
        for (final atx pack : packs) {
            ...
            try (final atc resources = pack.f()) {
                ...
                final atc unrwappedPackResources = PackUtil.unwrap(resources);
