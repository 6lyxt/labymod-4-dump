...
public class VersionedMinecraftSounds implements MinecraftSounds
{
    ...
    public void playChatFilterSound() {
        flk.Q().ak().a((hiq)hil.a((jr)awa.sw, 1.0f));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch) {
        this.playSound(hil.a(avz.a((akv)location.getMinecraftLocation(), 16.0f), pitch, volume));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch, final FloatVector3 position) {
        this.playSound(new hil((akv)location.getMinecraftLocation(), awb.a, volume, pitch, hiq.t(), false, 0, hiq.a.a, (double)position.getX(), (double)position.getY(), (double)position.getZ(), true));
    }
    
    @Override
    public void playSound(final ResourceLocation location, final float volume, final float pitch, final DoubleVector3 position) {
        this.playSound(new hil((akv)location.getMinecraftLocation(), awb.a, volume, pitch, hiq.t(), false, 0, hiq.a.a, position.getX(), position.getY(), position.getZ(), true));
    }
    
    private void playSound(final hil sound) {
        sound.a(flk.Q().ak());
        flk.Q().ak().a((hiq)sound);
    }
}
