...
public class VersionedTextureRepository extends DefaultAbstractTextureRepository implements TextureRepository
{
    ...
    public void onTick() {
        ...
        for (ResourceLocation resourceLocation : this.labyAPI.renderPipeline().resources().resourceLocationFactory().getCachedResourceLocations().values()) {
            if (resourceLocation instanceof final PathResourceLocation path) {
                final gpc textureManager = this.getTextureManager();
    ...
    public void register(final ResourceLocation location, final Texture texture, @Nullable final Runnable finishHandler) {
        final gom minecraftTexture = this.convertToVersionedTexture(texture);
    ...
    public void registerTexture(final ResourceLocation location, final Object texture, final Runnable finishHandler) {
        if (texture instanceof final gom minecraftTexture) {
        ...
        throw new IllegalStateException(texture.getClass().getName() + " is not a subtype of " + gom.class.getName());
    }
    
    @Override
    public void preloadTexture(final ResourceLocation location) {
        this.registerTexture(location, (gom)new gou((alf)location.getMinecraftLocation()));
    }
    ...
    public Texture getTexture(final ResourceLocation location) {
        ...
        final gom texture = this.getTextureManager().b((alf)location.getMinecraftLocation());
    ...
    private void registerTexture(@NotNull final ResourceLocation location, final gom texture) {
        this.getTextureManager().a((alf)location.getMinecraftLocation(), texture);
    }
    
    private void registerTexture(@NotNull final ResourceLocation location, final gom texture, final Runnable finishHandler) {
    ...
    private gom convertToVersionedTexture(final Texture texture) {
        if (texture instanceof final LabyTexture labyTexture) {
            return (gom)new VersionedLabyTexture(labyTexture);
        }
        if (texture instanceof final gom abstractTexture) {
            return abstractTexture;
        }
        throw this.createSubtypeException(texture, gom.class);
    }
}
