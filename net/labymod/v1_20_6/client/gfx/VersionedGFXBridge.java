...
import net.labymod.v1_20_6.client.gfx.buffer.VersionedBlaze3DBufferSource;
...
public class VersionedGFXBridge extends LWJGL3GFXBridge
{
    ...
    public void bindResourceLocation(final ResourceLocation location, final boolean blur, final boolean mipmap) {
        ...
        final gpc textureManager = ffh.Q().aa();
        final gom texture = textureManager.b((alf)location.getMinecraftLocation());
    ...
    public void setCursorPosition(double x, double y) {
        ...
        GLFW.glfwSetCursorPos(ffh.Q().aO().i(), Math.max(Math.min(x, window.getRawWidth()), 0.0), Math.max(Math.min(y, window.getRawHeight()), 0.0));
    }
