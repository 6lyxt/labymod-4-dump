...
public class StringConverter extends AbstractMinecraftWidgetConverter<fin, ComponentWidget>
{
    ...
    @Override
    public ComponentWidget createDefault(final fin source) {
    ...
    @Override
    public void update(final fin source, final ComponentWidget destination) {
        destination.setFocused(source.aH_());
        ...
        if (source instanceof final StringWidgetAccessor accessor) {
            if (accessor.isBasedOnTextWidth()) {
                source.k(ffh.Q().h.a((xu)source.y()));
            }
        }
        this.copyBounds(source, destination);
    }
    ...
    @Override
    public String getWidgetId(final fin source) {
        return this.componentMapper.getTranslationKeyOfComponent(source.y());
    }
    
    @Override
    public List<String> getWidgetIds(final fin source) {
