...
{
    private final fnf screen;
    private final fgt graphics;
    private Metadata metadata;
    private boolean initialized;
    
    public VersionedScreenWrapper(final fnf screen) {
        if (screen == null) {
            throw new NullPointerException("screen is null");
        }
        final ffh minecraft = ffh.Q();
        this.graphics = new fgt(minecraft, minecraft.aQ().c());
        this.screen = screen;
    ...
    public void initialize(final Parent parent) {
        if (parent != null) {
            this.screen.b(ffh.Q(), (int)parent.bounds().getWidth(BoundsType.INNER), (int)parent.bounds().getHeight(BoundsType.INNER));
        }
        else {
            final Window window = Laby.labyAPI().minecraft().minecraftWindow();
            this.screen.b(ffh.Q(), window.getScaledWidth(), window.getScaledHeight());
        }
    ...
    public void render(final Stack stack, final MutableMouse mouse, final float partialTicks) {
        ((GuiGraphicsAccessor)this.graphics).setPose((faa)stack.getProvider().getPoseStack());
    ...
    public void resize(final int width, final int height) {
        this.screen.b(ffh.Q(), width, height);
        this.initialized = true;
    }
    
    @Override
    public void render(final ScreenContext context) {
        ((GuiGraphicsAccessor)this.graphics).setPose((faa)context.stack().getProvider().getPoseStack());
