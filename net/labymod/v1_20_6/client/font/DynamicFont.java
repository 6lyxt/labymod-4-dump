...
package net.labymod.v1_20_6.client.font;
...
public class DynamicFont extends fgr implements FontHeightAccessor
{
    ...
    private final fgr delegate;
    private final ffs customSplitter;
    
    public DynamicFont(final fgr delegate) {
        super(delegate.f, delegate.g);
        this.delegate = delegate;
        this.customSplitter = new ffs((codepoint, style) -> DynamicFont.TEXT_RENDERER_PROVIDER.getRenderer().width(codepoint, (Style)style));
    }
    
    @NotNull
    public fjx a(@NotNull final alf location) {
    ...
    public int b(final String text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final Matrix4f modelViewMatrix, @NotNull final gdq bufferSource, @NotNull final fgr.a displayMode, final int backgroundColor, final int packedLightCoords, final boolean bidirectionalShaping) {
    ...
    public int b(@NotNull final ayl text, final float x, final float y, final int color, final boolean dropShadow, @NotNull final Matrix4f modelViewMatrix, @NotNull final gdq bufferSource, @NotNull final fgr.a displayMode, final int backgroundColor, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
    ...
    public void a(@NotNull final ayl text, final float x, final float y, final int color, final int outlineColor, @NotNull final Matrix4f modelViewMatrix, @NotNull final gdq bufferSource, final int packedLightCoords) {
        if (this.isForceVanillaFont()) {
        ...
        builder.pos(x, y).text(labyComponent).drawMode(MinecraftUtil.fromMinecraft(fgr.a.c)).color(color).shadow(false).render(stack);
        stack.pop();
    }
