...
package net.labymod.v1_20_6.client.font.text.vanilla;
...
import net.labymod.v1_20_6.client.font.DynamicStringRenderOutput;
...
public class VanillaTextRenderer extends AbstractTextRenderer
{
    ...
    public float width(final int codepoint, final Style style) {
        final ffs.f provider = ((StringSplitterAccessor)this.font().b()).getWidthProvider();
    ...
    public TextBuffer render(final Stack stack) {
        ...
        if (this.isAlphaAboveThreshold(color)) {
            ...
            if (!hasMultiBufferSource) {
                multiBufferSource = ffh.Q().aQ().c();
            }
            stringWidth = this.renderComponent(stack, (gdq)multiBufferSource, component, x, y, color);
            if (!hasMultiBufferSource) {
                ((gdq.a)multiBufferSource).b();
            }
        }
        else {
    ...
    public void beginBatch(final Stack stack) {
        if (stack.getMultiBufferSource() == null) {
            stack.multiBufferSource(ffh.Q().aQ().c());
    ...
    public void endBatch(final Stack stack) {
        if (this.bufferSourceAdded) {
            ...
            if (multiBufferSource instanceof final gdq.a bufferSource) {
                bufferSource.b();
            }
    ...
    private float renderComponent(final Stack stack, final gdq bufferSource, final RenderableComponent component, final float x, final float y, final int color) {
    ...
    private void renderText(final DynamicStringRenderOutput output, final ayl sequence, final float x, final float y, final int color, final boolean dropShadow, final Matrix4f modelViewMatrix) {
        ...
        if (dropShadow) {
            ...
            copiedModelViewMatrix.translate((Vector3fc)fgr.e);
        }
