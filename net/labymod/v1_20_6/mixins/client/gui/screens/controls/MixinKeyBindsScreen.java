...
public class MixinKeyBindsScreen
{
    ...
    private fff[] labyMod$checkKeys(final ffl instance) {
        final List<fff> mappings = new ArrayList<fff>(Arrays.asList(instance.W));
        for (final Hotkey hotkey : HotkeyHolder.getInstance().getHotkeys()) {
            mappings.add(LabyModKeyMapping.create(hotkey));
        }
        return mappings.toArray(new fff[0]);
    }
    ...
    private fff[] labyMod$resetKeys(final ffl instance) {
        final List<fff> mappings = new ArrayList<fff>(Arrays.asList(instance.W));
        for (final Hotkey hotkey : HotkeyHolder.getInstance().getHotkeys()) {
            mappings.add(LabyModKeyMapping.create(hotkey));
        }
