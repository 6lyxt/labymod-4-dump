...
@Mixin({ fot.class })
public abstract class MixinCreativeModeInventoryScreen extends fow<fot.b>
{
    ...
    public MixinCreativeModeInventoryScreen(final fot.b pickerMenu, final cmy inventory, final xp component) {
        super((cpw)pickerMenu, inventory, component);
    }
    ...
    private void labyMod$setEntityToOldPosition(final fgt graphics, final int x, final int y, final int width, final int height, final int scale, final float modelHeight, final float mouseX, final float mouseY, final btr entity) {
        if (this.labyMod$oldCreativeLayoutTexture) {
            fpe.a(graphics, x - this.labyMod$entityXShift, y - this.labyMod$entityYShift, width, height, scale, modelHeight, mouseX - this.labyMod$entityXShift, mouseY - this.labyMod$entityYShift, entity);
            return;
        }
        fpe.a(graphics, x, y, width, height, scale, modelHeight, mouseX, mouseY, entity);
    }
    ...
    private void labyMod$setOldCreativeTexture(final fgt graphics, final alf location, final int $$1, final int $$2, final int $$3, final int $$4, final int $$5, final int $$6) {
        if (!this.labyMod$oldCreativeLayoutTexture) {
    ...
    private void labyMod$onSelectTab(final ctc tab, final CallbackInfo ci) {
        this.labyMod$modifySlots();
    }
    
    private void labyMod$modifySlots() {
        if (this.labyMod$oldCreativeLayoutTexture) {
            for (final crr slot : ((fot.b)this.F()).i) {
    ...
    @NotNull
    private crr getSlotByIndex(@NotNull final InventoryLayoutOldAnimation inventoryLayout, @NotNull final InventorySlotData legacySlot, @NotNull final crr slot) {
