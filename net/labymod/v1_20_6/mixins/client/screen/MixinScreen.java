...
public abstract class MixinScreen implements FileDropHandler, ScreenAccessor
{
    ...
    public void fireScreenInitEvent(final ffh mc, final int width, final int height, final InsertInfo ci) {
        Laby.fireEvent(new VersionedScreenInitEvent(this));
    }
    
    @Insert(method = { "renderBackground" }, at = @At("HEAD"), cancellable = true)
    public void renderThemeBackground(final fgt graphics, final int mouseX, final int mouseY, final float partialTicks, final InsertInfo ci) {
    ...
    private void renderThemeBackground(Stack stack, final InsertInfo ci) {
        ...
        stack = ((stack != null) ? stack : ((VanillaStackAccessor)new faa()).stack());
    ...
    private void labyMod$hideMenuBackground(final fgt $$0, final CallbackInfo ci) {
    ...
    private void labyMod$disableMenuBackground(final fgt graphics, final int mouseX, final int mouseY, final float partialTicks, final InsertInfo ci) {
        if (Laby.labyAPI().config().appearance().hideMenuBackground().get()) {
            ci.cancel();
        }
    }
    ...
    public void renderComponentHoverEffect(final Stack stack, final Style style, final int x, final int y) {
        if (this.labyMod$graphics == null) {
            this.labyMod$graphics = new fgt(this.m, this.m.aQ().c());
        }
    ...
    @Override
    public void setMinecraft(final ffh minecraft) {
        this.m = minecraft;
    }
    ...
    public List<Object> getWrappedWidgets() {
        ...
        for (final fjc child : this.r) {
            wrappedWidgets.add(this.asVersionedWidget(child));
        }
        return wrappedWidgets;
    }
    
    private VersionedWidget asVersionedWidget(final fjc child) {
        ...
        if (child instanceof final fjb container) {
            for (final fjc entry : container.aD_()) {
