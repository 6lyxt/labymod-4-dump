...
public abstract class MixinGameRenderer
{
    ...
    private void labyMod$firePreRenderHandEvent(final fes camera, final float partialTicks, final Matrix4f viewMatrix, final CallbackInfo ci) {
    ...
    private void labyMod$firePostRenderHandEvent(final fes camera, final float partialTicks, final Matrix4f viewMatrix, final CallbackInfo ci) {
        final faa poseStack = MinecraftUtil.levelRenderContext().getPoseStack();
    ...
    public void labyMod$fireFieldOfViewTickEvent(final InsertInfo ci) {
        float modifier = 1.0f;
        final bsw an = this.k.an();
        if (an instanceof final gcp cameraEntity) {
    ...
    private boolean labyMod$noViewBobbing(final gdj instance, final faa stack, final float partialTicks) {
